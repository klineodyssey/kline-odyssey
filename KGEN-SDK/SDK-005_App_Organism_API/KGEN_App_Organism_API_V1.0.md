# SDK-005 KGEN App Organism API / App 生命體 API
**Version:** V1.0
...
|---|---|
| Document ID | SDK-005 |
| UUID | 90d93bc7-f887-59db-bc79-17ff4bfcf0f0 |
| Title | KGEN App Organism API |
| Version | V1.0 |
| Status | Draft for Review |
| Level | L5 Implementation |
| Author | PrimeForge / 樂天帝 ⌖ |
| Maintainer | KLINE ODYSSEY / PrimeForge |
| GitHub Path | `KGEN-SDK/SDK-005_App_Organism_API/KGEN_App_Organism_API_V1.0.md` |
...
| Attachment | Purpose |
|---|---|
| `schemas/sdk-005_schema.json` | JSON Schema for validation |
| `examples/sdk-005_example.json` | Example payload |
| `examples/sdk-005_types.ts` | TypeScript interface draft |
...
KGEN App Organism API 是 KGEN SDK / API Library V1.0 的正式草案。本冊把 Runtime Library 與 Machine-Readable Canon 轉成可供前端、AI Agent、文件工具與未來 SDK 使用的資料契約。
...
本 API 的核心 Entity 是 `AppOrganism`。其用途是：提供 App 生命體、DNA、技能、交易、租賃、組裝、合成與演化資料格式。 主要欄位包括：appId: string, dna: object, level: integer, lifeActions: string[]。
...
本冊提供 `schemas/sdk-005_schema.json`。Schema 用於檢查必要欄位、資料型別、版本、Document ID 與 Canon references。

## Chapter 6 / TypeScript Interface

本冊提供 `examples/sdk-005_types.ts` 作為 TypeScript interface 草案。草案用於前端與工具溝通，不代表已發布 npm package。

## Chapter 7 / Example Payload

本冊提供 `examples/sdk-005_example.json`。Example 只作資料格式示範，不是鏈上狀態保證，也不是未來收益承諾。
...
KGEN App Organism API 必須可追溯到對應 Runtime：GEN-001, GEN-002, GEN-003, GEN-004, GEN-006, GEN-007, GEN-008, GEN-009。若 Runtime 更新，SDK Schema 必須同步檢查。
...
當系統收到 `AppOrganism` payload 時，驗證器先確認 Document ID 與 version，再比對 KGEN-Canon 中的依賴圖，最後輸出可供 Codex 或 Cursor 使用的報告。
