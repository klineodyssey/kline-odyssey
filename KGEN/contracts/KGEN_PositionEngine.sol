...
contract KGEN_PositionEngine_V1_0_0 {
    ...
    function configureMarket(Market market,uint16 initialMarginBps,uint16 maintenanceMarginBps,uint32 maxOracleAge,uint256 minPriceWad,uint256 maxPriceWad,bool enabled) external onlyAdmin {
        if (initialMarginBps == 0 || maintenanceMarginBps == 0 || maintenanceMarginBps > initialMarginBps || initialMarginBps > BPS || maxOracleAge == 0 || minPriceWad == 0 || maxPriceWad <= minPriceWad) revert InvalidRiskConfig();
    ...
    function openPosition(address trader,Market market,int256 sizeWad,uint256 collateralWad) external onlyExecutor returns (uint256 positionId) {
        ...
        MarketConfig memory cfg=_config(market); (uint256 validated,,)=_readOracle(market); uint256 sizeAbs=_abs(sizeWad); uint256 notionalWad=KGEN_MarketRiskKernel_V1_0_0.notional(sizeAbs,validated); uint256 minInitialMargin=(notionalWad*cfg.initialMarginBps)/BPS; if (collateralWad<minInitialMargin) revert InitialMarginTooLow();
    ...
    function markPosition(uint256 positionId) public view returns (int256 unrealizedPnlWad,int256 equityWad,uint256 maintenanceMarginWad,bool liquidatable) {
        ...
        unrealizedPnlWad=KGEN_MarketRiskKernel_V1_0_0.pnl(p.sizeWad,p.entryPriceWad,mark); uint256 notionalWad=KGEN_MarketRiskKernel_V1_0_0.notional(_abs(p.sizeWad),mark); maintenanceMarginWad=KGEN_MarketRiskKernel_V1_0_0.maintenanceMargin(notionalWad,cfg.maintenanceMarginBps); equityWad=KGEN_MarketRiskKernel_V1_0_0.equity(int256(p.collateralWad),unrealizedPnlWad); liquidatable=KGEN_MarketRiskKernel_V1_0_0.liquidatable(p.collateralWad,unrealizedPnlWad,notionalWad,cfg.maintenanceMarginBps);
    }
    ...
    function _readOracle(Market market) internal view returns (uint256 priceWad,uint256 observedAt,uint8 validSources) {
        MarketConfig memory marketCfg=_config(market); OracleConfig storage oracleCfg=_oracleConfig[market]; if (oracleCfg.feedCount<2 || oracleCfg.minValidSources<2) revert InvalidOracleConfig(); uint256[3] memory prices; uint256 oldest=type(uint256).max;
        for (uint256 i=0;i<oracleCfg.feedCount;i++) {
            IKGENPriceFeedV1 feed=IKGENPriceFeedV1(oracleCfg.feeds[i]); uint8 decimalsValue; try feed.decimals() returns (uint8 d) { decimalsValue=d; } catch { continue; } if (decimalsValue>36) continue;
            try feed.latestRoundData() returns (uint80 roundId,int256 answer,uint256,uint256 updatedAt,uint80 answeredInRound) {
                if (answer<=0 || updatedAt==0 || updatedAt>block.timestamp || answeredInRound<roundId || block.timestamp-updatedAt>marketCfg.maxOracleAge) continue;
                uint256 normalized=_normalizeToWad(uint256(answer),decimalsValue); if (normalized<marketCfg.minPriceWad || normalized>marketCfg.maxPriceWad) continue; prices[validSources]=normalized; validSources++; if (updatedAt<oldest) oldest=updatedAt;
            } catch { continue; }
        }
        if (validSources<oracleCfg.minValidSources) revert OracleQuorumUnavailable();
        for (uint256 i=0;i<validSources;i++) for (uint256 j=i+1;j<validSources;j++) if (prices[j]<prices[i]) { uint256 tmp=prices[i]; prices[i]=prices[j]; prices[j]=tmp; }
        if (validSources==2) priceWad=(prices[0]/2)+(prices[1]/2)+((prices[0]%2+prices[1]%2)/2); else priceWad=prices[1];
        uint256 spread=prices[validSources-1]-prices[0]; if (spread>0 && (spread*BPS)/priceWad>oracleCfg.maxDeviationBps) revert OracleDeviationExceeded(); observedAt=oldest;
    }
