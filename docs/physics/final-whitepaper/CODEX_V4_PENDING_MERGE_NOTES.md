...
## 1. Do Not Treat This File as CURRENT

This file is a controlled merge queue only.

Codex must compare and reconcile:
...
under the existing single-source reconciliation instructions.
...
KGEN ≠ carbon
KGEN ≠ iron
KGEN ≠ a periodic-table element
KGEN ≠ confirmed physical dark matter
...
```text
Code Size
= bytes, source lines, model parameters or stored data volume
...
→ Organism / App
→ Micro-Civilization
→ Habitat / Planetary Civilization
→ Larger Civilization
```
...
```text
A. Observed / established real-world physics
B. Open scientific hypothesis
C. KGEN engineering mapping or civilization axiom
```
...
- [ ] distinguishes real physics, hypothesis and KGEN engineering axioms;
- [ ] references the KAIOS Life Runtime and 11520 Universe Exchange Runtime;
- [ ] does not create a new parallel physics master file.
...
Until that record is filled, this file remains a pending merge queue and not a canonical physics source.
