(() => {
  ...
  function findExpiredActiveClaims(workerRegistry, now = Date.now()) {
    const activeClaims = workerRegistry?.active_claims || [];
    return activeClaims.filter((claim) => {
      if (!["CLAIMED", "IN_PROGRESS"].includes(String(claim.status || ""))) return false;
  ...
  function evaluateAlerts(tasks, reviews, config, sourceData) {
    ...
    for (const claim of findExpiredActiveClaims(sourceData.workerRegistry)) {
      addAlert("danger", "Expired active claim drift", `${claim.claim_id || claim.task_id} expired at ${claim.lease_expires_at}; no automatic close was performed.`);
    }
  }
  ...
  function renderLeaderState(config, buildInfo, workerRegistry) {
    const leader = config.repository_leader_tree || {};
    const mainSha = buildInfo?.origin_main_commit || config.base_main_commit || "WARN";
    const leaderHead = buildInfo?.source_commit || "READ_FROM_GITHUB_PR_HEAD";
    const leaderTree = buildInfo?.source_tree || "READ_FROM_GIT_TREE";
    const ahead = buildInfo?.source_ahead_of_main;
    const behind = buildInfo?.source_behind_main;
    const relation = Number.isInteger(ahead) && Number.isInteger(behind)
      ? `ahead ${ahead} / behind ${behind}`
      : "READ_FROM_GITHUB_PR";
    const expired = findExpiredActiveClaims(workerRegistry);

    $("leaderMainSha").textContent = mainSha;
    $("leaderBranch").textContent = leader.leader_branch || "UNASSIGNED";
    $("leaderHead").textContent = leaderHead;
    $("leaderTree").textContent = leaderTree;
    $("leaderVsMain").textContent = relation;
    $("leaderGate").textContent = `${leader.ci_status_source || "UNKNOWN"} / ${leader.review_status || "UNKNOWN"}`;
    $("leaderReleaseStatus").textContent = leader.release_status || "UNKNOWN";
    $("queueDriftStatus").textContent = expired.length ? "DRIFT_FOUND" : "CONSISTENT";
    $("expiredClaimCount").textContent = `${expired.length}`;
    $("expiredClaims").innerHTML = expired.length
      ? expired.map((claim) => `<li><strong>EXPIRED · FAIL CLOSED</strong> ${escapeHtml(claim.claim_id || claim.task_id)} · ${escapeHtml(claim.lease_expires_at)} · no automatic close</li>`).join("")
      : "<li><strong>NO EXPIRED ACTIVE CLAIM</strong> Registry snapshot contains no expired CLAIMED/IN_PROGRESS lease.</li>";
  }
  ...
  async function boot() {
    ...
    evaluateAlerts(state.tasks, state.reviews, config, { buildInfo, workerRegistry, workQueue, claimSchema, statusSchema });
    renderHealth(config, buildInfo, state.reviews);
    renderPublicRelease(config, buildInfo);
    renderLeaderState(config, buildInfo, workerRegistry);
