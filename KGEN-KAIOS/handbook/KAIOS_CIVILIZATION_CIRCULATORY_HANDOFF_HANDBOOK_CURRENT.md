...
- #188 exact head `f4f9ebbc86f3a078e8914bb3f247b5c75f7d32bd` cumulatively integrates #169, #178, #179, #180, #181 and #185 on current main. It preserves the assigned `0.00011520` K11520 company/KGEN-price coordinate without seeding matched-trade CT, combines the paper market, listing/warehouse gates and browser-safe mobile wallet, and adds read-only GPU readiness plus a settlement-interface observation. Fixed, closed-schema repository sources truthfully record zero accepted GPU inventory records and zero funded segregated trading-capital accounts. Those sources are not external trust authorities: non-empty self-labelled records are rejected and `REPOSITORY_BOUND_GPU_EVIDENCE_VERIFIER_NOT_WIRED` remains unconditional. At BSC block `118394469` (`2026-08-27T13:49:36Z`), two fixed HTTPS providers agreed on the ExchangeSettlement11520 address, EIP-1967 implementation address `0xA08A9CEcfa18b2FDb9ca8De0063A5029B9Ffc363`, 18888 bank address, fixed 11520 address, getter results and `totalSettled=0`. Expected runtime code hashes are not repository-bound, so address/storage/getter agreement does not prove source-code equivalence or the configured funding-only capability claim. The exact identity status must remain `BLOCKED_SETTLEMENT_CODE_IDENTITY_NOT_REPOSITORY_BOUND`; independently, the observed interface is `INCOMPATIBLE_WITH_ATOMIC_GPU_TRADE_SETTLEMENT`, and the production GPU settlement adapter remains `NOT_IMPLEMENTED`. The mobile Token page has no GPU trade, signing or settlement control. Caller-supplied RPC URLs or fetch functions remain schema probes. Mother Machine evidence proves only `CONNECTED_BINDING_ONLY / VERIFIED_BOUND`, not transaction, treasury or trading authority; the transaction Policy Broker remains unconnected. Local tests are 286/286 PASS, local mobile browser QA is clean, and exact-head push CI `33079055022`, PR CI `33079060506` and Product QA `33079060590` are green, but those checks do not remove this P0 code-identity blocker. #188 remains Draft and still requires distinct review, repository-bound code identity, independently verified inventory, funded segregated capital, production market/settlement, a reviewed Policy Broker and no-broadcast fork evidence.
