...
interface IKUFOCarrierBurnRecordSource {
...
/**
 * @title KSHIP
 * @notice Immutable KSHIP antimatter core born at K108000 with a registry-gated mass-energy burn port.
...
contract KSHIP is ERC20, ERC20Capped {
        ...
        ERC20Capped(MAX_SUPPLY)
    {
        ...
        kufo = IKUFOCarrierBurnRecordSource(kufoToken);
    }
    ...
    function mintFromCarrierProof(bytes32 proofId) external returns (address beneficiary, uint256 amount) {
        ...
        IKUFOCarrierBurnRecordSource.CarrierBurnRecord memory burnRecord = kufo.carrierBurnRecord(proofId);
        if (
