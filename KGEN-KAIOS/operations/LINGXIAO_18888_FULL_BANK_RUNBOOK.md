...
12. Generate the chain-derived Genesis record and unsigned immutable-inscription deployment with `node tools/validate-integration-artifacts.mjs --generate-kaios-genesis-record --tx <settlementTx> --kaios <KAIOS> --kgen <KGEN> --bank <18888Proxy> --furnace <18911> --registry <Registry> --deployer <deploymentSigner>`. Never type a supply, burn or mint amount. The tool validates the successful receipt and exact chain state, confirms all five addresses contain code, preserves frozen deployments 1-21, requires the deployment signer's pending nonce to remain 55, and writes `reports/mainnet-pre-sign/KAIOS_GENESIS_INSCRIPTION_UNSIGNED_DEPLOYMENT.json` without signing or sending it.
13. Obtain separate Human authorization for the inscription deployment. Deploy `KAIOSGenesisInscription` only after Genesis evidence validation as deployment action 22. Verify its constants, five code-bearing addresses, quantities, settlement hash/block, deployment block/timestamp, runtime bytecode and absence of mutable authority.
