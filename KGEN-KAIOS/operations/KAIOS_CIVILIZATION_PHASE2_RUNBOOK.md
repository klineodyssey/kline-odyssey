...
The reorg-aware index covers the original module ABI events plus formal 18888 `ModuleConfigured`, BankGovernance `GovernanceProposalCreated`, `GovernanceProposalApproved`, `GovernanceProposalExecuted`, `GovernanceProposalCancelled`, and KGEN `SetTaxWallets`, `OwnershipTransferred`, and `Transfer`. KGEN ownership is reconstructed from canonical `OwnershipTransferred` history ordered by block number, transaction index, and log index; the latest canonical event wins, removed logs are ignored, and replay starts from event history rather than a hardcoded deployer. The read model independently compares the indexed owner with `KGEN.owner()`. A mismatch produces `ERROR_MISMATCH` and disables governance-sensitive UI actions. A KGEN `Transfer` into ReserveRedemption is recorded as `UNCLASSIFIED_KGEN_INFLOW` unless separate deterministic evidence establishes attribution; it must not be mislabeled as tax merely because of its recipient.

Stage-2C ownership-index regression evidence was refreshed on BSC Mainnet fork block `116004980`. The replay updated the owner from `0xb3C54ca96De0dED4Ca0151F629ff9781506ba261` to BankGovernance `0xa2792fBDCc8A8AaC364053431D44E0a8D335E166`, matched the fork RPC `owner()` result, processed a later governed transfer to a second address, reconstructed the governed tax-wallet redirect while ReserveRedemption remained disabled, and failed closed when indexed ownership was deliberately made stale. No Mainnet transaction was sent.
