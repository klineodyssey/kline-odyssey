...
class CursorWorkerRegistryRepairTests(unittest.TestCase):
    ...
    def test_fruit_tree_candidate_dispatch_is_the_only_active_claim(self):
        locked = validate_one_task_lock(self.registry["dispatch_history"])
        self.assertEqual(len(locked), 1)
        dispatch = locked[0]
        self.assertEqual(
            dispatch["task_id"], "KAIOS-CURSOR-FRUIT-TREE-PACKAGES-001"
        )
        self.assertEqual(dispatch["worker_id"], self.cursor["worker_id"])
        self.assertEqual(
            dispatch["branch"],
        ...
        self.assertIn("FRUIT_TREE_PACKAGE", self.cursor["allowed_work"])

        crop_dispatch = next(
            item
            for item in self.registry["dispatch_history"]
            if item["task_id"] == "KAIOS-CURSOR-CROP-LIFE-PACKAGES-001"
        )
        self.assertEqual(crop_dispatch["status"], "RELEASED")
    ...
    def test_continuous_queue_requires_formal_release_and_atomic_claim(self):
        ...
        self.assertEqual(by_priority[3]["status"], "DISPATCHED")
        self.assertEqual(by_priority[3]["task_id"], self.cursor["current_task"])
