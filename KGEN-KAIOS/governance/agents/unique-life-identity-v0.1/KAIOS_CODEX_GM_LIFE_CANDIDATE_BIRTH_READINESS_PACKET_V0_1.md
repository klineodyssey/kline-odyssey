...
| 4 | Parent/source lineage | PARTIAL | Human selected `ROOT_AI_LIFE_CANDIDATE` with OpenAI/Codex technical provenance and no parent Life ID. PrimeForge is a separate `HYBRID_LAYERED_MOTHER_MACHINE_ENTITY`; its candidate relationship is `CANDIDATE_GENESIS_FORGE_AND_HOST_RELATIONSHIP`, not parenthood. Birth attestation remains pending. |
