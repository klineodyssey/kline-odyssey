...
const K280_YEAR_SECONDS = 31_556_926;
...
test("18911 burn is holder-allowance-bound and 511111 cannot redirect beneficiary", async () => {
  ...
  await assert.rejects(
    context.furnace.connect(context.treasury).burnForKufo(
      ...
      lifeId,
      await context.furnace.DESTINATION_511111(),
    ),
  );
  ...
  const created = eventArgs(receipt, context.furnace, "ImmediateAlchemyCreated");
...
test("511111 wormhole births KUFO immediately and the proof remains single-use", async () => {
  const context = await setupLineage();
  ...
  assert.equal(proof.destinationCode, await context.furnace.DESTINATION_511111());
  assert.equal(await context.kufo.balanceOf(await context.treasury.getAddress()), amount * 1_000n);
  await assert.rejects(context.wormhole.releaseImmediate(proofId));
});

test("KUFO to KSHIP conversion is maturity-bound, replay-safe, and conserves scale", async () => {
  const context = await setupLineage();
  await mintKaiosByBurningKgen(context, 1n * ETHER);
  const kaiosAmount = 400n * ETHER;
  const recipient = context.signers[2];
  await (await context.kgen.transfer(await context.treasury.getAddress(), kaiosAmount / 1_000n)).wait();
  ...
  eventArgs(burnReceipt, context.furnace, "ImmediateAlchemyCreated");
  ...
  await advanceTime(context.provider, K280_YEAR_SECONDS);
  await (
    await context.converter.connect(recipient).convert(
      kufoAmount,
