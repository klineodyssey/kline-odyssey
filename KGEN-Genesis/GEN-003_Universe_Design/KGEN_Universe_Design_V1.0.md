...
GEN-003《KGEN 宇宙設計書》是 KGEN Genesis Library V1.0 的正式出版文件。本文件狀態為 **Draft for Review**，層級為 **L3 Design Bible**，目的在於：整理 Universe Map、K-Sphere、CT 時間市場、座標與神殿分佈。
本書以 UniverseMap_V10_2_DISTANCE_COMPLETE_ALL_POINTS.json 作為世界骨架，承認 123 個座標概念與 16888 = 384,400 km 的比例參考。
...
KGEN Universe Design 的架構與整體 KGEN 宇宙架構一致：Universe 產生 Civilization，Civilization 展開 World，World 形成 Temple 群，Temple 連接 Land、Building、App、AI、DNA、Runtime 與 Code。
正式架構不允許以重複資料夾、重複 Runtime、版本號正式器官、patch/fix/hotfix/stable/final 類正式檔名來替代既有器官。若相同功能已存在，必須先閱讀與合併，而不是新增平行世界。
本書特定焦點為：Universe Map、K-Sphere、CT Time Market、Portal。這些焦點必須回到同一套 Universe Runtime，而不能成為互相衝突的系統。
...
本文件正式路徑為 `KGEN-Genesis/GEN-003_Universe_Design/KGEN_Universe_Design_V1.0.md`。同目錄同步提供 DOCX、PDF、README 與 assets/README.md。
