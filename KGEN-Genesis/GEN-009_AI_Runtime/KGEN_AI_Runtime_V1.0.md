...
補完 AI 權限、AI 不得修改 Canon、AI 如何讀文件、如何協作 Cursor/Codex、如何產生 Runtime、如何避免亂創作。
...
1. Chapter 1 / 第1章 AI Runtime 總綱
2. Chapter 2 / 第2章 AI 權限
3. Chapter 3 / 第3章 必讀順序
4. Chapter 4 / 第4章 Codex / Cursor 協作
5. Chapter 5 / 第5章 Runtime 產生規則
6. Chapter 6 / 第6章 Canon 防護
7. Chapter 7 / 第7章 文件生成規則
8. Chapter 8 / 第8章 程式修改規則
9. Chapter 9 / 第9章 索引更新
10. Chapter 10 / 第10章 錯誤處理
11. Chapter 11 / 第11章 安全邊界
12. Chapter 12 / 第12章 交付流程
...
## Chapter 1 / 第1章 AI Runtime 總綱

本章確立《KGEN AI Runtime 規範》在 KGEN Genesis Library V1.0 中的正式位置。本書不是暫時筆記，也不是單頁說明，而是可被官網、GitHub、AI、SDK、Whitepaper、CMC、CoinGecko、BscScan 與社群資料引用的官方典藏文件。

本書的權限為 **L4 Runtime**。它服務的讀者包含：Codex、Cursor、Autopilot、AI agent、人類 reviewer 與 Runtime 維護者。。它的核心任務是：補完 AI 權限、AI 不得修改 Canon、AI 如何讀文件、如何協作 Cursor/Codex、如何產生 Runtime、如何避免亂創作。

本書的權威邊界是：本書是 AI 進入 KGEN repo 前必讀的行為規範之一，受 Boot V1.4 與 AGENTS.md 約束。 若本書與下層實作衝突，應先檢查 GEN-001、GEN-002、GEN-004、Runtime CURRENT 與實作文件，再決定是修正文案、更新索引，或退回該實作。
...
對本書而言，這套哲學落在以下具體命題：補完 AI 權限、AI 不得修改 Canon、AI 如何讀文件、如何協作 Cursor/Codex、如何產生 Runtime、如何避免亂創作。 因此本書不能只列項目，它必須說明定義、規則、案例、流程、限制與風險。
...
| Term | Definition |
|---|---|
| AI Runtime | AI 的工作流程與權限邊界，不是新的宇宙法則。 |
| Read Order | AI 開始任何修改前必須遵守的文件讀取順序。 |
| Controlled Generation | AI 可生成文件、索引、SDK 與實作，但必須先檢查同功能既存檔案。 |
...
案例：Cursor 修 UI 時若新增 kgen-12345-ui-v3.js，Codex 應治理為固定正式器官檔名 kgen-12345-ui.js，版本寫入檔內。
...
## Chapter 11 / 第11章 安全邊界

本章把本書落到檔案與流程。正式 Markdown 路徑為 `KGEN-Genesis/GEN-009_AI_Runtime/KGEN_AI_Runtime_V1.0.md`，同步 DOCX 與 PDF 位於同一目錄。每本書的 README 說明檔案用途，assets/README.md 說明資產狀態。
...
案例：AI 若要補文件，應更新 Genesis Library 與索引，而不是另開一個未登錄資料夾。

## Chapter 12 / 第12章 交付流程
...
本書風險焦點：AI 可能產生錯誤引用、重複檔案、過度創作或與 Canon 衝突的內容，因此必須透過索引、讀取順序與人工授權控制。

限制：不得把 AI 幻覺當成官方 Canon。; 不得跳過保護路徑檢查。; 不得刪除舊白皮書內容來製作差異版。。任何違反限制的內容都應被標記、退回或修正，而不是直接合併進正式典藏。
...
- AI 不得修改 Canon；AI 必須遵守 Genesis；AI 必須遵守 Constitution；AI 可擴充但不可推翻官方設定。
- AI 修改前必須讀 Boot V1.4、Runtime CURRENT、Universe Map、AGENTS.md、README、目標文件。
- AI 不得任意建立新版 Runtime、patch/fix/hotfix/stable 正式檔、重複 bootstrap 或 Universe Runtime。

本書流程：讀取 → 掃描 → 比對既有功能 → 設計最小變更 → 更新索引 → 驗證 → 回報 → 經授權 commit/push。
...
本文件可獨立閱讀。讀者即使尚未讀過其他 GEN 文件，也可以透過本書理解 KGEN AI Runtime 規範 的定義、規則、流程、限制與風險；若需要查證來源，請依 Dependencies 與 Related Documents 追溯。
