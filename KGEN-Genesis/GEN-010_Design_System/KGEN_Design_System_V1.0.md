...
|---|---|
| Document ID | GEN-010 |
| UUID | e27b8722-3bbe-5dca-902b-be559603fd58 |
| Title | KGEN Design System / KGEN 設計系統 |
| Version | V1.0 |
| Status | Official V1.0 Full Canon Set |
| Level | L3 Design Bible |
| Author | PrimeForge / 樂天帝 ⌖ |
| Maintainer | KLINE ODYSSEY / PrimeForge |
| GitHub Path | KGEN-Genesis/GEN-010_Design_System/KGEN_Design_System_V1.0.md |
| Last Update | 2026-07-09 |

## Document Purpose

補完品牌、Logo、黑金宇宙風、神殿 UI、圖像規則、一圖一神殿、文件排版規範與命名規範。
...
| GEN-010 | KGEN Design System / KGEN 設計系統 | L3 Design Bible | [KGEN_Design_System_V1.0.md](./KGEN_Design_System_V1.0.md) |
...
1. Chapter 1 / 第1章 設計總綱
2. Chapter 2 / 第2章 品牌語氣
3. Chapter 3 / 第3章 Logo 規則
4. Chapter 4 / 第4章 黑金宇宙風
5. Chapter 5 / 第5章 神殿 UI
6. Chapter 6 / 第6章 一圖一神殿
7. Chapter 7 / 第7章 Token Icon
8. Chapter 8 / 第8章 文件排版
9. Chapter 9 / 第9章 命名規範
10. Chapter 10 / 第10章 資產治理
11. Chapter 11 / 第11章 實體品牌連結
12. Chapter 12 / 第12章 審核素材
...
本章確立《KGEN 設計系統》在 KGEN Genesis Library V1.0 中的正式位置。本書不是暫時筆記，也不是單頁說明，而是可被官網、GitHub、AI、SDK、Whitepaper、CMC、CoinGecko、BscScan 與社群資料引用的官方典藏文件。

本書的權限為 **L3 Design Bible**。它服務的讀者包含：品牌設計者、前端工程師、文件出版者、神殿視覺維護者與平台資產審核者。。它的核心任務是：補完品牌、Logo、黑金宇宙風、神殿 UI、圖像規則、一圖一神殿、文件排版規範與命名規範。

本書的權威邊界是：本書定義 KGEN 官方視覺與出版品質。 若本書與下層實作衝突，應先檢查 GEN-001、GEN-002、GEN-004、Runtime CURRENT 與實作文件，再決定是修正文案、更新索引，或退回該實作。
...
對本書而言，這套哲學落在以下具體命題：補完品牌、Logo、黑金宇宙風、神殿 UI、圖像規則、一圖一神殿、文件排版規範與命名規範。 因此本書不能只列項目，它必須說明定義、規則、案例、流程、限制與風險。

## Chapter 3 / 第3章 Logo 規則

本章定義本書使用的官方詞彙。所有詞彙都必須回到 Genesis、Canon、Constitution、Runtime CURRENT 與 Universe Map，不得各自發明互相衝突的語言。

| Term | Definition |
|---|---|
| Black-Gold Universe | KGEN 官方黑金宇宙視覺語言，代表文明、金融、神殿與 Web3 官方性。 |
| One Image One Temple | 一圖一神殿，正式神殿圖像不得混用。 |
| Publication Layout | Markdown、DOCX、PDF、封面、表格與索引的一致出版格式。 |
...
案例：12345 永遠使用悟空財神殿相關圖片，不得混入 16888 廣寒宮或月下老人內容。
...
## Chapter 11 / 第11章 實體品牌連結

本章把本書落到檔案與流程。正式 Markdown 路徑為 `KGEN-Genesis/GEN-010_Design_System/KGEN_Design_System_V1.0.md`，同步 DOCX 與 PDF 位於同一目錄。每本書的 README 說明檔案用途，assets/README.md 說明資產狀態。
...
案例：BscScan token icon 必須在 32x32 GitHub Preview 清楚可見，不能只剩中央小點。

## Chapter 12 / 第12章 審核素材
...
本書風險焦點：錯誤視覺會造成平台審核混亂、玩家誤認、神殿身份混淆與品牌信任下降。

限制：不得混用神殿圖片。; 不得用普通 README 風格取代官方首頁與出版物。; 不得用 patch/final/hotfix 檔名作為正式器官。。任何違反限制的內容都應被標記、退回或修正，而不是直接合併進正式典藏。
...
- Logo、token icon、神殿圖、首頁 Hero、文件封面與 PDF 必須維持官方辨識一致。
- 每座神殿使用自己的正式圖片與視覺語言，不得共享造成誤認。
- 正式器官檔名固定，版本寫入檔案內容或文件 metadata，不寫入正式檔名。

本書流程：品牌原則 → 圖像來源 → 神殿綁定 → UI 元件 → 文件出版 → 外部平台資產包。
...
本文件可獨立閱讀。讀者即使尚未讀過其他 GEN 文件，也可以透過本書理解 KGEN 設計系統 的定義、規則、流程、限制與風險；若需要查證來源，請依 Dependencies 與 Related Documents 追溯。
