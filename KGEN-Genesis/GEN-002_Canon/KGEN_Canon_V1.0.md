...
GEN-002《KGEN 官方正典》是 KGEN Genesis Library V1.0 的正式出版文件。本文件狀態為 **Draft for Review**，層級為 **L1 Canon**，目的在於：固定不可違反的世界觀、公理與生命鏈。
本書是官方正典。任何神殿、AI、SDK 或遊戲規格若與 Canon 衝突，必須修正或退回。
...
KGEN Canon 的架構與整體 KGEN 宇宙架構一致：Universe 產生 Civilization，Civilization 展開 World，World 形成 Temple 群，Temple 連接 Land、Building、App、AI、DNA、Runtime 與 Code。
正式架構不允許以重複資料夾、重複 Runtime、版本號正式器官、patch/fix/hotfix/stable/final 類正式檔名來替代既有器官。若相同功能已存在，必須先閱讀與合併，而不是新增平行世界。
本書特定焦點為：Canon、不可違反、正典審查、擴充不推翻。這些焦點必須回到同一套 Universe Runtime，而不能成為互相衝突的系統。
...
本文件正式路徑為 `KGEN-Genesis/GEN-002_Canon/KGEN_Canon_V1.0.md`。同目錄同步提供 DOCX、PDF、README 與 assets/README.md。
