...
GEN-012《KGEN 開發者 SDK 規範》是 KGEN Genesis Library V1.0 的正式出版文件。本文件狀態為 **Draft for Review**，層級為 **L5 Implementation**，目的在於：定義開發者如何讀取 Boot、Runtime、Universe Map、AGENTS、文件索引、神殿與合約資料。
本書給外部開發者與 AI 一個穩定入口，說明如何讀文件、查路徑、引用神殿與遵守 Runtime。
...
KGEN Developer SDK 的架構與整體 KGEN 宇宙架構一致：Universe 產生 Civilization，Civilization 展開 World，World 形成 Temple 群，Temple 連接 Land、Building、App、AI、DNA、Runtime 與 Code。
正式架構不允許以重複資料夾、重複 Runtime、版本號正式器官、patch/fix/hotfix/stable/final 類正式檔名來替代既有器官。若相同功能已存在，必須先閱讀與合併，而不是新增平行世界。
本書特定焦點為：SDK、Developer onboarding、API surface、Repository map。這些焦點必須回到同一套 Universe Runtime，而不能成為互相衝突的系統。
...
本文件正式路徑為 `KGEN-Genesis/GEN-012_Developer_SDK/KGEN_Developer_SDK_V1.0.md`。同目錄同步提供 DOCX、PDF、README 與 assets/README.md。
