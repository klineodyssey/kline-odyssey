...
【V8.8.2_hotfix（只增補，不改核心）】
- 產出 dated 檔後，同步覆蓋更新 master latest：--master 指向的台指近全.xlsx
  （因 autopilot_all.yml 會 test -f TX_MASTER，必須存在且更新）
- 結算價=0 判定：只以「非空且==0」視為結算，避免空值被當作 0 而誤判。
"""
...
def _pick_near_month_for_day(g: pd.DataFrame) -> int:
    """
    每日近月判定（V8.8.2 增補規則）：以「日盤結算價=0」視為結算日 → 排除該月。
    注意：只看「非空且==0」，不把空值當 0，避免誤判。
    """
    ...
    for m in months:
        ...
        settle_non_na = settle.dropna()

        # 結算日：日盤結算價真的出現 0 → 排除該月
        if (settle_non_na == 0).any():
            continue
...
def _cli():
    ...
    if last_master_out:
        ...
        # (2) latest：覆蓋更新 --master 指向的檔（workflow 需要）
        if master_path:
            master.to_excel(master_path, index=False, sheet_name="台指近全")
            _freeze_first_row_and_autowidth(master_path, sheet_name="台指近全")
            print(master_path)
