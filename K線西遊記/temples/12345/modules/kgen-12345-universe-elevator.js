...
/* =========================================================
   KGEN 12345 MASTER STABLE LOCK V10.31
   Purpose:
   - Fix amount input ownership: no auto 8, no forced value, no focus jump.
   - Restore and protect heartbeat / ignite / vowTo / lightLamp / makeWish transaction buttons.
   - Make left XY MOVE actually move main image X/Y.
   - Make Universe Elevator and MOVE Y share the same Y source.
   - Keep mirror mode: order/click triggers bull front mirror or bear rear mirror by current direction.
   - Lock a single visible version source.
   ========================================================= */
(function(){
  'use strict';
  const VERSION='12345-TEMPLE-V10.31-MASTER-STABLE-LOCK';
  const BUILD='2026/05/16 V10.31 final stable lock';
  const d=document, root=d.documentElement;
  const $=id=>d.getElementById(id);
  const qa=s=>Array.from(d.querySelectorAll(s));
  const clamp=(v,a,b)=>Math.max(a,Math.min(b,Number(v)||0));
  const state=window.KGEN12345_COORDINATE_STATE || (window.KGEN12345_COORDINATE_STATE={x:0,elevator:20,deg:0,mirror:'idle'});
  function speak(msg){try{ if(!window.speechSynthesis) return; const u=new SpeechSynthesisUtterance(String(msg||'')); u.lang='zh-TW'; u.rate=1; speechSynthesis.cancel(); speechSynthesis.speak(u);}catch(_){}}
  function status(msg){['bp-status','last-event','kh-log','kgen-v1030-status'].forEach(id=>{const el=$(id); if(el) el.textContent=String(msg||'');});}
  function css(){
    if($('kgen-v1031-stable-lock-css')) return;
    const st=d.createElement('style'); st.id='kgen-v1031-stable-lock-css';
    st.textContent=`
      :root{--kgen-main-x:0px;--kgen-main-y:0px;--kgen-core-deg:0deg;--kgen-elevator-percent:6.666%;}
      #core-anchor{transform:translate(calc(-50% + var(--kgen-main-x)), calc(-50% + var(--kgen-main-y)))!important;transition:transform .12s linear!important;will-change:transform!important;}
      #core-window{transform:rotate(var(--kgen-core-deg))!important;transition:transform .12s linear!important;will-change:transform!important;}
      #fairy-img{user-select:none!important;-webkit-user-drag:none!important;}
      #move-joystick-wrap{pointer-events:auto!important;touch-action:none!important;z-index:2147482500!important;}
      #move-joystick-knob{pointer-events:none!important;}
      #warp-rail-body,.warp-rail,[class*="elevator"],[id*="elevator"]{touch-action:none;}
      input[data-kgen-amount],#kgen-12345-amount-input,#amt-in,#kh-amount,#kh-amount-master,#fortune-amount{pointer-events:auto!important;touch-action:manipulation!important;user-select:text!important;-webkit-user-select:text!important;position:relative!important;z-index:2147482600!important;}
      .kgen-v1031-tx-grid{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important;margin-top:10px!important;}
      .kgen-v1031-tx-grid button,.kgen-v1031-tx-grid input{min-height:42px!important;border-radius:10px!important;}
      .kgen-v1031-note{grid-column:1/3;color:#9ff;font-size:12px;font-weight:800;line-height:1.45;border:1px solid rgba(0,242,255,.25);border-radius:10px;padding:8px;background:rgba(0,242,255,.06)}
      body.kgen-v1031-bull #core-window{box-shadow:0 0 80px rgba(0,242,255,.34),0 0 40px rgba(255,215,120,.25)!important;}
      body.kgen-v1031-bear #core-window{box-shadow:0 0 80px rgba(255,80,80,.34),0 0 40px rgba(255,215,120,.18)!important;}
      #kgen-v1031-version-lock{position:fixed;left:12px;top:6px;z-index:2147482000;color:#61fff6;font:700 10px/1.2 monospace;opacity:.01;pointer-events:none;}
      #kgen-v1030-collapse-all{top:46px!important;right:8px!important;z-index:2147482000!important;}
    `;
    d.head.appendChild(st);
  }
  function syncVersion(){
    window.KGEN_12345_VERSION=VERSION; window.KGEN_12345_BUILD=BUILD;
    if(!$('kgen-v1031-version-lock')){const x=d.createElement('div');x.id='kgen-v1031-version-lock';x.textContent=VERSION;d.body.appendChild(x);}
    qa('*').forEach(el=>{
      if(el.children.length>0) return;
      const t=(el.textContent||'');
      if(/VERSION\s+12345/i.test(t)||/V10\.(24|25|26|27|30)/.test(t)){
        if(t.length<120) el.textContent=t.replace(/VERSION\s+12345[-_A-Z0-9\. ]*/i,'VERSION '+VERSION).replace(/V10\.(24|25|26|27|30)[-_A-Z0-9\. ]*/g,'V10.31-MASTER-STABLE-LOCK');
      }
    });
  }
  function amountInputs(){
    return Array.from(new Set(qa('input').filter(i=>{
      const type=(i.type||'').toLowerCase(); if(['range','checkbox','radio','button','submit','file'].includes(type)) return false;
      const id=(i.id||'').toLowerCase(), ph=(i.placeholder||'').toLowerCase(), nm=(i.name||'').toLowerCase();
      return /amount|amt|kgen|kh-amount/.test(id+' '+ph+' '+nm) || i.hasAttribute('data-kgen-amount');
    })));
  }
  function setupAmount(){
    amountInputs().forEach(i=>{
      i.type='text'; i.inputMode='decimal'; i.autocomplete='off'; i.setAttribute('data-kgen-amount','1'); i.setAttribute('data-kgen-user-owned','true');
      i.placeholder=i.placeholder||'請自行輸入 KGEN 數量';
      // 系統不得自動填 8 或其他數字。只有使用者輸入才同步。
      if(!i.dataset.kgenUserTyped && String(i.value||'').trim()==='8') i.value='';
      if(i.dataset.kgenV1031Amount==='1') return; i.dataset.kgenV1031Amount='1';
      ['pointerdown','mousedown','touchstart','click','focus','keydown','keyup'].forEach(ev=>i.addEventListener(ev,e=>{e.stopPropagation();},true));
      i.addEventListener('input',()=>{
        i.dataset.kgenUserTyped='1';
        i.value=String(i.value||'').replace(/[，,]/g,'.').replace(/[^0-9.]/g,'').replace(/(\..*)\./g,'$1');
        const v=i.value;
        amountInputs().forEach(o=>{ if(o!==i && d.activeElement!==o) o.value=v; });
        status('金額已由操作者輸入：'+(v||'空白')+' KGEN');
      },true);
    });
  }
  function getAmount(){const f=amountInputs().find(i=>String(i.value||'').trim()); return f?String(f.value).trim():'';}
  function yFromElevator(e){ return Math.round((clamp(e,0,300)-20)*1.4); }
  function applyMove(){
    const e=clamp(state.elevator,0,300); const y=yFromElevator(e); const x=clamp(state.x,-220,220);
    root.style.setProperty('--kgen-main-x',Math.round(x)+'px'); root.style.setProperty('--kgen-main-y',y+'px');
    root.style.setProperty('--v72-move-x',Math.round(x)+'px'); root.style.setProperty('--v72-move-y',y+'px');
    root.style.setProperty('--kgen-v1027-main-x',Math.round(x)+'px'); root.style.setProperty('--kgen-v1027-main-y',y+'px');
    root.style.setProperty('--kgen-elevator-percent',(e/300*100).toFixed(3)+'%');
    ['kgen-v1027-elevator-value','kgen-v1026-elevator-value','warp-val','warp-label'].forEach(id=>{const el=$(id); if(el) el.textContent=String(Math.round(e));});
    const knob=$('move-joystick-knob'); if(knob){knob.style.left=(40+clamp(x/2.8,-38,38))+'px'; knob.style.top=(40+clamp((20-e)/280*38,-38,38))+'px';}
    const lab=$('move-joystick-label'); if(lab) lab.textContent='X '+Math.round(x)+' / Y '+Math.round(e)+' / 電梯 '+Math.round(e);
    const core=$('core-anchor'); if(core){ core.style.transform=`translate(calc(-50% + ${Math.round(x)}px), calc(-50% + ${y}px))`; }
    updateStatus();
  }
  function setElevator(v){state.elevator=clamp(v,0,300); applyMove();}
  function setupMove(){
    const wrap=$('move-joystick-wrap'); if(!wrap||wrap.dataset.kgenV1031Move==='1') return; wrap.dataset.kgenV1031Move='1';
    let active=false,pid=null;
    function handle(cx,cy){
      const r=wrap.getBoundingClientRect(); const mx=r.left+r.width/2, my=r.top+r.height/2;
      let dx=cx-mx, dy=cy-my; const lim=Math.min(r.width,r.height)*0.34; const len=Math.hypot(dx,dy)||1;
      if(len>lim){dx=dx/len*lim; dy=dy/len*lim;}
      state.x=clamp(dx/lim*120,-120,120);
      state.elevator=clamp(20+(-dy/lim)*280,0,300);
      applyMove(); setHeartActive();
    }
    wrap.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); active=true;pid=e.pointerId;try{wrap.setPointerCapture(pid)}catch(_){} handle(e.clientX,e.clientY);},true);
    wrap.addEventListener('pointermove',e=>{if(!active||e.pointerId!==pid)return; e.preventDefault();e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); handle(e.clientX,e.clientY);},true);
    function up(e){if(pid!==null&&e&&e.pointerId!==pid)return;active=false;pid=null;}
    wrap.addEventListener('pointerup',up,true); wrap.addEventListener('pointercancel',up,true); wrap.addEventListener('lostpointercapture',up,true);
  }
  function setupElevator(){
    const candidates=[$('warp-rail-body'),...qa('.warp-rail,[id*="elevator"],[class*="elevator"]')].filter(Boolean);
    const rail=candidates.find(el=>{const r=el.getBoundingClientRect(); return r.height>120 && r.width<260;});
    if(!rail||rail.dataset.kgenV1031Rail==='1') return; rail.dataset.kgenV1031Rail='1';
    function setFromY(clientY){const r=rail.getBoundingClientRect(); setElevator((1-clamp((clientY-r.top)/Math.max(1,r.height),0,1))*300); setHeartActive();}
    rail.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); try{rail.setPointerCapture(e.pointerId)}catch(_){} setFromY(e.clientY);},true);
    rail.addEventListener('pointermove',e=>{if(e.buttons!==1)return; e.preventDefault();e.stopPropagation(); if(e.stopImmediatePropagation)e.stopImmediatePropagation(); setFromY(e.clientY);},true);
  }
  function currentDeg(){const s=$('steer-input-val'); const v=s?Number(s.value||0):state.deg||0; return Number.isFinite(v)?v:0;}
  function applyDeg(){state.deg=currentDeg(); root.style.setProperty('--kgen-core-deg',state.deg+'deg'); const cw=$('core-window'); if(cw) cw.style.transform='rotate('+state.deg+'deg)';}
  function isBull(){const a=currentDeg(); return a>=-90 && a<=90;}
  function setMirror(side, reason){
    const bull=side==='bull'||(side!=='bear'&&isBull()); state.mirror=bull?'bull':'bear';
    d.body.classList.toggle('kgen-v1031-bull',bull); d.body.classList.toggle('kgen-v1031-bear',!bull); d.body.classList.add('kgen-camera-on');
    status((reason||'下單')+'｜鏡頭啟動：'+(bull?'多方前鏡':'空方後鏡'));
  }
  let heartTimer=null;
  function setHeartActive(){
    d.body.classList.add('kgen-heart-active'); clearTimeout(heartTimer); heartTimer=setTimeout(()=>d.body.classList.remove('kgen-heart-active'),650);
  }
  function clickByIdOrText(id, words, label){
    const a=$(id); if(a){a.click(); setMirror(null,label); return;}
    const btn=qa('button,a').find(b=>words.some(w=>w.test(b.textContent||'')));
    if(btn){btn.click(); setMirror(null,label); return;}
    status(label+'：找不到原始下單按鈕，請檢查合約面板。'); speak(label+'找不到原始下單按鈕。');
  }
  function ensureTempleOps(){
    const old=window.templeOps||{};
    window.templeOps=Object.assign({},old,{
      approve: old.approve || (()=>clickByIdOrText('kh-approve',[/approve|授權/i],'Approve 授權')),
      cup: old.cup || (()=>{try{window.KGEN12345_HOLY_CUP?.tap?.();}catch(_){} status('三聖盃已請示。');}),
      fortune: old.fortune || (()=>clickByIdOrText('kh-fortune',[/fortuneClaim|發財金|領發財金/i],'fortuneClaim 發財金')),
      heartbeat: old.heartbeat || (()=>clickByIdOrText('kh-heartbeat',[/heartbeatClaim|整點心跳|心跳/i],'heartbeatClaim 整點心跳')),
      ignite: old.ignite || (()=>clickByIdOrText('kh-ignite',[/igniteAndClaim|轉日呼吸|呼吸/i],'igniteAndClaim 轉日呼吸')),
      vow: old.vow || (()=>clickByIdOrText('kh-vow',[/vowTo|還願/i],'vowTo 還願')),
      lamp: old.lamp || (()=>clickByIdOrText('kh-lamp',[/lightLamp|點燈/i],'lightLamp 點燈')),
      wish: old.wish || (()=>clickByIdOrText('kh-wishbtn',[/makeWish|許願/i],'makeWish 許願'))
    });
  }
  function ensureControlButtons(){
    const panel=$('web3-panel')||$('kgen-heart-live-panel')||qa('[id*="heart"],[id*="web3"]').find(Boolean); if(!panel) return;
    let dock=$('kgen-v1031-final-dock');
    if(!dock){
      dock=d.createElement('div'); dock.id='kgen-v1031-final-dock'; dock.className='kgen-v1031-tx-grid';
      dock.innerHTML=`<div class="kgen-v1031-note">悟空控制台：金額由操作者自行輸入；系統不自動填 8。心跳、呼吸、許願、還願、點燈全部保留。</div>
        <input id="kh-amount-master" data-kgen-amount="1" placeholder="請自行輸入 KGEN 數量">
        <button type="button" onclick="templeOps.approve()">Approve 授權</button>
        <button type="button" onclick="templeOps.fortune()">fortuneClaim 發財金</button>
        <button type="button" onclick="templeOps.heartbeat()">heartbeatClaim 心跳</button>
        <button type="button" onclick="templeOps.ignite()">igniteAndClaim 呼吸</button>
        <button type="button" onclick="templeOps.vow()">vowTo 還願下單</button>
        <button type="button" onclick="templeOps.lamp()">lightLamp 點燈下單</button>
        <button type="button" style="grid-column:1/3" onclick="templeOps.wish()">makeWish 許願下單</button>`;
      panel.appendChild(dock);
    }
    setupAmount();
  }
  function updateStatus(){const el=$('kgen-v1030-status')||$('bp-status'); if(el) el.textContent='MASTER V10.31｜Amount '+(getAmount()||'操作者自行輸入')+'｜X '+Math.round(state.x)+'｜Elevator/Y '+Math.round(state.elevator)+'｜'+(isBull()?'多方前鏡':'空方後鏡');}
  function bindOrderMirror(){['kh-fortune','kh-heartbeat','kh-ignite','kh-vow','kh-lamp','kh-wishbtn'].forEach(id=>{const b=$(id); if(b&&!b.dataset.kgenV1031Mirror){b.dataset.kgenV1031Mirror='1'; b.addEventListener('click',()=>setMirror(null,'鏈上交易'),true);}}); qa('button').forEach(b=>{const t=b.textContent||''; if(/fortuneClaim|heartbeatClaim|igniteAndClaim|vowTo|lightLamp|makeWish|發財金|心跳|呼吸|還願|點燈|許願/.test(t)&&!b.dataset.kgenV1031Mirror){b.dataset.kgenV1031Mirror='1'; b.addEventListener('click',()=>setMirror(null,'交易器官'),true);}});}
  function init(){css(); syncVersion(); setupAmount(); ensureTempleOps(); ensureControlButtons(); setupMove(); setupElevator(); applyDeg(); applyMove(); bindOrderMirror(); const s=$('steer-input-val'); if(s&&!s.dataset.kgenV1031Deg){s.dataset.kgenV1031Deg='1'; s.addEventListener('input',()=>{applyDeg(); setMirror(null,'多空方向切換');},true);} status('V10.31 MASTER STABLE LOCK 已啟動。');}
  if(d.readyState==='loading') d.addEventListener('DOMContentLoaded',init); else init();
  window.addEventListener('load',init); setTimeout(init,600); setTimeout(init,1800); setTimeout(init,3600);
  setInterval(()=>{syncVersion();setupAmount();ensureTempleOps();ensureControlButtons();bindOrderMirror();updateStatus();},2500);
  window.KGEN12345_MASTER_LOCK={version:VERSION,state,setElevator,setXY:(x,y)=>{state.x=clamp(x,-220,220);state.elevator=clamp(y,0,300);applyMove();},getAmount,setMirror};
})();
