...
function enforceOpenSurfaceLayer(){
  for(const el of document.querySelectorAll('#confirm.open,.sheet.open,.confirm.open')){
    ...
    el.style.setProperty('z-index',String(TOP_Z),'important');
...
export function install11520MobileActionRailClearance(){
  ...
  document.addEventListener('click',e=>{if(e.target?.closest?.('#orderFire,#attack,.order,.attack')){queueMicrotask(apply);requestAnimationFrame(apply);setTimeout(apply,60)}},{capture:true});
