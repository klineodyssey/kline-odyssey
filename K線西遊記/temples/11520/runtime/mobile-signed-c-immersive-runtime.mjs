...
.k11520NumericEditor{position:absolute;left:50%;bottom:-24px;transform:translateX(-50%);width:48px;height:20px;z-index:4;border:1px solid #68e4ff55;border-radius:6px;background:#071018f2;color:#e9fbff;text-align:center;padding:0 2px;font:800 8px ui-monospace,SFMono-Regular,Consolas,monospace;outline:none;touch-action:manipulation}
.k11520NumericEditor:focus{border-color:#dffaff;box-shadow:0 0 0 1px #dffaff55}
#cNumericInput{color:var(--k11520-neutral-color)}
#cControl[data-c-sign="positive"] #cNumericInput{color:var(--k11520-long-color)}
#cControl[data-c-sign="negative"] #cNumericInput{color:var(--k11520-short-color)}
...
function normalizeNumericC(raw){const n=Number(String(raw).replace(/C/ig,'').trim());if(!Number.isFinite(n))return null;const bounded=Math.max(-1000,Math.min(1000,n));if(Math.abs(bounded)<0.0000005)return 0;const magnitude=ABS_LEVELS[nearestAbsIndex(Math.abs(bounded))];return Math.sign(bounded)*magnitude}
...
function dispatchNativeLots(lots){const el=$('#lotsControl');if(!el)return;const r=el.getBoundingClientRect(),t=Math.max(.01,Math.min(1,Number(lots)/100)),y=r.top+(1-t)*r.height,x=r.left+r.width/2;for(const [type,buttons] of [['pointerdown',1],['pointerup',0]])el.dispatchEvent(new PointerEvent(type,{bubbles:true,pointerId:912,pointerType:'touch',clientX:x,clientY:y,buttons}))}
function readLots(){const n=parseInt($('#lotsRead')?.textContent||'1',10);return Number.isFinite(n)?Math.max(1,Math.min(100,n)):1}
function syncNumericEditors(){const c=$('#cNumericInput'),lots=$('#lotsNumericInput');const value=Number(signedByAxis[activeAxis()]||0);if(c&&document.activeElement!==c)c.value=formatInputC(value);if(lots&&document.activeElement!==lots)lots.value=String(readLots())}
function setCFromNumeric(raw){const normalized=normalizeNumericC(raw);if(normalized==null){syncNumericEditors();return null}applySignedValue(normalized);syncNumericEditors();return normalized}
function setLotsFromNumeric(raw){const n=Number(String(raw).replace(/口/g,'').trim());if(!Number.isFinite(n)){syncNumericEditors();return null}const lots=Math.max(1,Math.min(100,Math.round(n)));dispatchNativeLots(lots);queueMicrotask(syncNumericEditors);setTimeout(syncNumericEditors,40);publish();return lots}
function bindNumericEditor(input,commit){if(input.dataset.k11520NumericBound)return;input.dataset.k11520NumericBound='1';for(const type of ['pointerdown','pointermove','pointerup','pointercancel','touchstart','touchmove','touchend'])input.addEventListener(type,e=>e.stopPropagation(),{capture:false,passive:true});input.addEventListener('keydown',e=>{e.stopPropagation();if(e.key==='Enter'){e.preventDefault();commit(input.value);input.blur()}else if(e.key==='Escape'){e.preventDefault();syncNumericEditors();input.blur()}});input.addEventListener('change',()=>commit(input.value));input.addEventListener('blur',()=>{commit(input.value);syncNumericEditors()})}
function ensureNumericEditors(){const cControl=$('#cControl'),lotsControl=$('#lotsControl');if(!cControl||!lotsControl)return false;let c=$('#cNumericInput');if(!c){c=document.createElement('input');c.id='cNumericInput';c.className='k11520NumericEditor';c.type='text';c.inputMode='decimal';c.autocomplete='off';c.spellcheck=false;c.setAttribute('aria-label','C 速度精準輸入；可輸入正負值');c.title='輸入 -1000 ～ +1000；依目前 C 能階取最近可用值';cControl.appendChild(c)}let lots=$('#lotsNumericInput');if(!lots){lots=document.createElement('input');lots.id='lotsNumericInput';lots.className='k11520NumericEditor';lots.type='number';lots.inputMode='numeric';lots.min='1';lots.max='100';lots.step='1';lots.autocomplete='off';lots.setAttribute('aria-label','口數精準輸入；1 到 100 正整數');lots.title='輸入 1 ～ 100 口；口數永遠為正';lotsControl.appendChild(lots)}bindNumericEditor(c,setCFromNumeric);bindNumericEditor(lots,setLotsFromNumeric);syncNumericEditors();return true}
...
function paintSignedC(axis=activeAxis()){
  ...
  renderTradeSideLock();syncNumericEditors();publish();return true;
}
...
function renderTradeSideLock(){const side=$('#sideBtn');if(!side)return;const value=Number(signedByAxis[activeAxis()]||0),wanted=signSide(value),text=wanted==='LONG'?'多':wanted==='SHORT'?'空':'中性',nextText=`方向：${text}（由 ${formatC(value)}）`,nextTitle='多空由 C 正負唯一決定；不可另外切換';if(side.textContent!==nextText)side.textContent=nextText;if(!side.disabled)side.disabled=true;if(side.dataset.k11520Side!==wanted)side.dataset.k11520Side=wanted;if(side.getAttribute('aria-disabled')!=='true')side.setAttribute('aria-disabled','true');if(side.title!==nextTitle)side.title=nextTitle}
function onCEvent(e){if(internalNative||e.target?.classList?.contains('k11520NumericEditor'))return;const el=e.target?.closest?.('#cControl');if(!el)return;if(e.type==='pointerdown'){cPointer=e.pointerId;try{el.setPointerCapture?.(cPointer)}catch{}}if(e.type==='pointermove'&&e.pointerId!==cPointer)return;if((e.type==='pointerup'||e.type==='pointercancel')&&e.pointerId!==cPointer)return;e.preventDefault();e.stopImmediatePropagation();if(e.type!=='pointerup'&&e.type!=='pointercancel'){const v=signedFromPointer(e.clientY,el.getBoundingClientRect());applySignedValue(v)}else cPointer=null}
function onDocumentClick(e){if(e.target?.id==='sideBtn'){e.preventDefault();e.stopImmediatePropagation();renderTradeSideLock();return}const card=e.target?.closest?.('#axes [data-axis]');if(card)setTimeout(()=>{paintSignedC(card.dataset.axis);void syncCanonicalSide(Number(signedByAxis[card.dataset.axis]||0));syncNumericEditors()},0)}
