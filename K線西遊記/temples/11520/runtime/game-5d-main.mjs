...
bindVertical('#lotsControl',t=>{axis().lots=Math.max(1,Math.round(t*100));syncControls()});bindVertical('#cControl',t=>{const vals=[0,.000001,.00001,.0001,.001,.01,.1,1,10,100];axis().c=vals[Math.round(t*(vals.length-1))];syncControls()});
...
function openOrder(){syncTradeAxisFromPlane();const a=axis(),p=price();if(!p){toast('行情未就緒');return}const principal=requiredMargin({lots:a.lots}),risk=positionRisk({entry:p,mark:p,side:a.side,lots:a.lots,c:a.c});pending={axis:S.axis,market:a.market,side:a.side,lots:a.lots,c:a.c,entry:p,principal};const perPercent=pending.lots*Math.abs(pending.c)/100,adversePercent=risk.maxAdverseFraction*100;$('#confirmBody').innerHTML=`<div class="grid2"><div class="card"><h3>${pending.axis} ${pending.market}</h3>${pending.side} · ${pending.lots}口 · ${pending.c}C<br>本金 ${pending.principal} KGEN</div><div class="card"><h3>風險</h3>每 1% 變動 ±${fmt(perPercent,6)} KGEN<br>反向歸零 ${Number.isFinite(adversePercent)?fmt(adversePercent,4)+'%（'+fmt(risk.maxAdversePoints,3)+' 價格點）':'∞'}</div></div><p class="bad">目前確認只建立本機模擬部位，不送鏈。</p>`;$('#confirm').classList.add('open')}
