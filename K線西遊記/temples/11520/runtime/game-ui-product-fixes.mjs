...
VERSION: 2.4.3
STATUS: ACTIVE
PURPOSE: Persistent two-state joystick: tap toggles XZ/Y; selected mode stays active until the next intentional tap.
*/
import {install11520ProductFixes as installV23} from './game-ui-product-fixes-v23.mjs';

const $=s=>document.querySelector(s);
const MODE_KEY='k11520.joystick.mode.v24';
let yMode=false,yPid=null,yInterval=null,yDir=0,tapPid=null,tapStart=null,tapMoved=false,tapAt=0;
try{yMode=sessionStorage.getItem(MODE_KEY)==='Y'}catch{}
...
#joyModeV24{position:absolute;left:0;right:0;top:-22px;text-align:center;font-size:8px;font-weight:900;color:#68e4ff;pointer-events:none;white-space:nowrap}
...
@media(max-width:420px){#joy{--guideSize:98px;--knobSize:48px}}
...
function stopY(){clearInterval(yInterval);yInterval=null;yDir=0}
function updateUi(){const joy=$('#joy'),knob=$('#knob'),m=$('#joyModeV24'),up=$('#joyV24Up'),dn=$('#joyV24Down'),lt=$('#joyV24Left'),rt=$('#joyV24Right');if(!joy||!knob)return;joy.dataset.controlMode=yMode?'Y':'XZ';knob.textContent=yMode?'Y':'K';knob.setAttribute('aria-label',yMode?'Y 模式；上下拖曳控制高度；輕點切 XZ':'XZ 模式；拖曳控制移動；輕點切 Y');if(m)m.textContent=yMode?'Y MODE · 上下控制 Y · 輕點切 XZ':'XZ MODE · 拖曳控制 X/Z · 輕點切 Y';if(up)up.textContent=yMode?'Y+':'Z+';if(dn)dn.textContent=yMode?'Y−':'Z−';if(lt)lt.textContent='X−';if(rt)rt.textContent='X+'}
function setMode(next){stopY();yPid=null;yMode=!!next;try{sessionStorage.setItem(MODE_KEY,yMode?'Y':'XZ')}catch{}updateUi()}
...
function installOverride(){const joy=$('#joy'),knob=$('#knob');if(!joy||!knob||joy.dataset.v24YInstalled)return false;joy.dataset.v24YInstalled='1';ensureStyle();installLabels();
  ...
  joy.addEventListener('pointerup',e=>{if(e.pointerId!==tapPid&&e.pointerId!==yPid)return;const intentionalTap=e.pointerId===tapPid&&!tapMoved&&performance.now()-tapAt<350;if(yMode&&e.pointerId===yPid){e.preventDefault();e.stopImmediatePropagation();yPid=null;stopY()}if(e.pointerId===tapPid){tapPid=null;tapStart=null;tapMoved=false;if(intentionalTap)setMode(!yMode)}},{capture:true});
...
function updateAiCopy(){const msgs=$('#aiMsgs');if(!msgs||msgs.dataset.v243ControlHelp)return;msgs.dataset.v243ControlHelp='1';const n=document.createElement('div');n.className='aiMsg';n.textContent='V2.4 控制：輕點遙桿可在 XZ / Y 間切換；選定後會保持該模式。XZ 拖曳控制 X/Z；Y 上下控制 Y+/Y−；只有再次輕點才換模式。';msgs.appendChild(n)}
export function install11520ProductFixes(){const knob=$('#knob');if(knob)knob.dataset.yCenterInstalled='v24-owner';const result=installV23();const ready=()=>{const k=$('#knob');if(k)k.dataset.yCenterInstalled='v24-owner';stampVersion();updateAiCopy();return installOverride()};if(!ready()){const mo=new MutationObserver(()=>{if(ready())mo.disconnect()});mo.observe(document.documentElement,{childList:true,subtree:true})}return{...result,version:'2.4.3',features:[...(result?.features||[]).filter(x=>x!=='left-center-y'),'persistent-xz-y-toggle','thumb-first-xz','direct-y-touch','aligned-joystick-rings','legacy-center-y-disabled']}}
