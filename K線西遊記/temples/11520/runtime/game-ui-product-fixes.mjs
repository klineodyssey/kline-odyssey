...
VERSION: 2.4.0
STATUS: ACTIVE
PURPOSE: Preserve V2.3 product fixes while replacing legacy center single/double-tap Y control with XZ<->Y mode toggle and 0.5s hold Y+/Y- control.
*/
import {install11520ProductFixes as installV23} from './game-ui-product-fixes-v23.mjs';

const $=s=>document.querySelector(s);
let yMode=false,yPid=null,yTimer=null,yInterval=null,yDir=0;
...
function updateUi(){const joy=$('#joy'),knob=$('#knob'),m=$('#joyModeV24'),up=$('#joyV24Up'),dn=$('#joyV24Down'),lt=$('#joyV24Left'),rt=$('#joyV24Right');if(!joy||!knob)return;joy.dataset.controlMode=yMode?'Y':'XZ';knob.textContent=yMode?'Y':'K';if(m)m.textContent=yMode?'Y MODE · 按住 0.5 秒：上 Y+ / 下 Y−':'XZ MODE · 點中央 K 切換 Y';if(up)up.textContent=yMode?'Y+':'Z+';if(dn)dn.textContent=yMode?'Y−':'Z−';if(lt)lt.textContent='X−';if(rt)rt.textContent='X+'}
function setMode(next){stopY();yMode=!!next;updateUi()}
...
function installOverride(){const joy=$('#joy'),knob=$('#knob');if(!joy||!knob||joy.dataset.v24YInstalled)return false;joy.dataset.v24YInstalled='1';ensureStyle();installLabels();
  knob.addEventListener('pointerdown',e=>{e.preventDefault();e.stopImmediatePropagation()},{capture:true});
  knob.addEventListener('pointerup',e=>{e.preventDefault();e.stopImmediatePropagation();setMode(!yMode)},{capture:true});
  ...
  for(const ev of ['pointerup','pointercancel'])joy.addEventListener(ev,e=>{if(!yMode||e.pointerId!==yPid)return;e.preventDefault();e.stopImmediatePropagation();yPid=null;stopY()},{capture:true});
  return true}
...
function updateAiCopy(){const input=$('#aiInput');if(!input)return;}

export function install11520ProductFixes(){const result=installV23();const ready=()=>{stampVersion();return installOverride()};if(!ready()){const mo=new MutationObserver(()=>{if(ready())mo.disconnect()});mo.observe(document.documentElement,{childList:true,subtree:true})}return{...result,version:'2.4.0',features:[...(result?.features||[]),'xz-y-mode-toggle','y-hold-500ms']}}
