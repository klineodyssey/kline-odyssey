...
let mode='XZ',xyPid=null,xyProxyPid=null,lastKnobDown=null,fallbackCenterDown=null,syncQueued=false;
...
#knob{z-index:8!important;cursor:pointer!important;overflow:hidden!important;pointer-events:auto!important}
...
function expectedImage(){return mode==='XY'?{src:FAIRY,alt:'XY 模式仙女'}:{src:KGEN,alt:'XZ 模式 KGEN'}}
function ensureKnobImage(){const knob=$('#knob');if(!knob)return false;let img=knob.querySelector('img');if(!img){img=document.createElement('img');img.draggable=false;knob.replaceChildren(img)}const exp=expectedImage();if(img.getAttribute('src')!==exp.src)img.setAttribute('src',exp.src);if(img.getAttribute('alt')!==exp.alt)img.setAttribute('alt',exp.alt);if(img.style.display!=='block')img.style.display='block';if(img.style.visibility!=='visible')img.style.visibility='visible';if(img.style.opacity!=='1')img.style.opacity='1';return true}
function render(){ensureLabels();ensureKnobImage();const label=$('#k11520PlaneLabel'),top=$('#k11520DirTop'),bottom=$('#k11520DirBottom'),left=$('#k11520DirLeft'),right=$('#k11520DirRight');const set=(el,v)=>{if(el&&el.textContent!==v)el.textContent=v};set(label,`${mode} 模式 · 點中央圖切換`);set(left,'X−');set(right,'X+');set(top,mode==='XY'?'Y+':'Z+');set(bottom,mode==='XY'?'Y−':'Z−');document.documentElement.dataset.k11520JoyPlane=mode;globalThis.__K11520_JOYSTICK_PLANE__={version:'2.6.9',mode,persistent:true,horizontal:'X',vertical:mode==='XY'?'Y':'Z'}}
function scheduleSync(){if(syncQueued)return;syncQueued=true;queueMicrotask(()=>{syncQueued=false;render()})}
function installImageGuard(){const knob=$('#knob');if(!knob||knob.dataset.k11520PlaneImageGuard)return;knob.dataset.k11520PlaneImageGuard='1';const mo=new MutationObserver(()=>{const img=knob.querySelector('img'),exp=expectedImage();if(!img||img.getAttribute('src')!==exp.src||img.getAttribute('alt')!==exp.alt)scheduleSync()});mo.observe(knob,{childList:true,subtree:true,attributes:true,attributeFilter:['src','alt']});globalThis.__K11520_JOYSTICK_IMAGE_GUARD__=mo}
...
function toggle(){mode=mode==='XZ'?'XY':'XZ';saveMode();render();for(const delay of [16,80,180])setTimeout(render,delay);flash(mode==='XY'?'XY 模式：左右 X、上下 Y':'XZ 模式：左右 X、上下 Z')}
...
function insideKnobPoint(e,knob){const r=knob.getBoundingClientRect();return e.clientX>=r.left&&e.clientX<=r.right&&e.clientY>=r.top&&e.clientY<=r.bottom}
function bind(){const joy=$('#joy'),knob=$('#knob');if(!joy||!knob||joy.dataset.k11520PlaneBound)return false;joy.dataset.k11520PlaneBound='1';
  knob.addEventListener('pointerdown',e=>{lastKnobDown={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now()}},{capture:true,passive:true});
  knob.addEventListener('pointerup',e=>{const s=lastKnobDown;lastKnobDown=null;if(!s||s.id!==e.pointerId)return;if(Math.hypot(e.clientX-s.x,e.clientY-s.y)<=8&&performance.now()-s.t<420){e.preventDefault();e.stopPropagation();fallbackCenterDown=null;toggle()}},{capture:true,passive:false});
  knob.addEventListener('pointercancel',()=>{lastKnobDown=null},{capture:true});
  joy.addEventListener('pointerdown',e=>{if(!e.__k11520XYProxy&&!e.target.closest?.('#knob')&&insideKnobPoint(e,knob))fallbackCenterDown={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now()};if(mode!=='XY'||e.__k11520XYProxy)return;xyPid=e.pointerId;xyProxyPid=900000+(Number(e.pointerId)||1);queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointerdown',e,xyProxyPid)})},{capture:true,passive:true});
  joy.addEventListener('pointermove',e=>{const f=fallbackCenterDown;if(f&&f.id===e.pointerId&&Math.hypot(e.clientX-f.x,e.clientY-f.y)>8)fallbackCenterDown=null;if(mode!=='XY'||e.__k11520XYProxy||e.pointerId!==xyPid)return;queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointermove',e,xyProxyPid)})},{capture:true,passive:true});
  const end=e=>{const f=fallbackCenterDown;fallbackCenterDown=null;if(f&&f.id===e.pointerId&&!e.target.closest?.('#knob')&&insideKnobPoint(e,knob)&&Math.hypot(e.clientX-f.x,e.clientY-f.y)<=8&&performance.now()-f.t<420){e.preventDefault();e.stopPropagation();toggle()}if(e.__k11520XYProxy||e.pointerId!==xyPid)return;const pid=xyProxyPid;xyPid=null;xyProxyPid=null;queueMicrotask(()=>yEvent(e.type==='pointercancel'?'pointercancel':'pointerup',e,pid||999999))};
...
function install(){style();loadMode();ensureLabels();render();bind();installImageGuard();let n=0;const tick=()=>{n++;render();bind();installImageGuard();if(n<20)setTimeout(tick,180)};setTimeout(tick,80);return globalThis.__K11520_JOYSTICK_PLANE__}
export function install11520JoystickPlaneModeV269(){return install()}
