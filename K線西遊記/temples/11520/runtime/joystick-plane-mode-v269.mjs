...
let mode='XZ',xyPid=null,xyProxyPid=null,lastKnobDown=null,fallbackCenterDown=null;
...
/* Former Y UI is retired visually, but #yControl keeps real geometry offscreen so the canonical Y setter can be reused by XY mode. */
...
function ensureLabels(){const joy=$('#joy');if(!joy)return false;for(const [id,cls] of [['k11520PlaneLabel',''],['k11520DirTop',''],['k11520DirBottom',''],['k11520DirLeft',''],['k11520DirRight','']])if(!$('#'+id)){const n=document.createElement('span');n.id=id;n.className=cls;joy.appendChild(n)}return true}
...
function render(){ensureLabels();ensureKnobImage();const label=$('#k11520PlaneLabel'),top=$('#k11520DirTop'),bottom=$('#k11520DirBottom'),left=$('#k11520DirLeft'),right=$('#k11520DirRight');if(label)label.textContent=`${mode} 模式 · 點中央圖切換`;if(left)left.textContent='X−';if(right)right.textContent='X+';if(top)top.textContent=mode==='XY'?'Y+':'Z+';if(bottom)bottom.textContent=mode==='XY'?'Y−':'Z−';document.documentElement.dataset.k11520JoyPlane=mode;globalThis.__K11520_JOYSTICK_PLANE__={version:'2.6.9',mode,persistent:true,horizontal:'X',vertical:mode==='XY'?'Y':'Z'}}
...
function install(){style();loadMode();ensureLabels();render();bind();let n=0;const tick=()=>{n++;render();bind();if(n<20)setTimeout(tick,180)};setTimeout(tick,80);return globalThis.__K11520_JOYSTICK_PLANE__}
