...
VERSION: 2.5.7
STATUS: ACTIVE
PURPOSE: Runtime visual repair for the human-tested 11520 mobile layout. Keeps canonical control behavior while fixing missing XZ art, compacting the wallet valve, separating Y from combat, keeping the open dock rail clear of the standalone backpack, sizing control imagery, synchronizing version labels, and retiring the known lookPad interceptor. No trade, wallet, chain, settlement, payment, treasury, or governance behavior changes.
*/
...
function ensureStyle(){
  if($('#k11520ControlsV257Style'))return;
  for(const id of ['k11520ControlsV251Style','k11520ControlsV252Style','k11520ControlsV253Style','k11520ControlsV254Style','k11520ControlsV255Style','k11520ControlsV256Style'])$('#'+id)?.remove();
  const s=document.createElement('style');s.id='k11520ControlsV257Style';s.textContent=`
  ...
  #joy #knob .v257ControlImg{width:100%!important;height:100%!important;display:block!important;object-fit:cover!important;border-radius:50%!important;pointer-events:none!important;user-select:none!important}
  ...
  #yJoyV250 .yKnob .v257ControlImg{width:100%!important;height:100%!important;display:block!important;object-fit:cover!important;object-position:center!important;border-radius:50%!important;pointer-events:none!important;user-select:none!important}
  ...
  #cControl .thumb .v257ControlImg,#lotsControl .thumb .v257ControlImg{width:100%!important;height:100%!important;display:block!important;object-fit:contain!important;border-radius:50%!important;pointer-events:none!important;user-select:none!important}
  ...
  @media(max-width:420px){
    ...
    .dock,.dock.open{right:5px!important;bottom:max(10px,env(safe-area-inset-bottom))!important;z-index:3000!important}.dock.open .rail{right:0!important;bottom:136px!important;z-index:3001!important;max-height:calc(100vh - 154px)!important}
...
function putImage(host,src,alt){if(!host)return false;host.querySelectorAll('img.v252ControlImg,img.v255ControlImg,img.v256ControlImg,img.v257ControlImg').forEach((img,i)=>{if(i)img.remove()});let img=host.querySelector('img.v257ControlImg')||host.querySelector('img.v256ControlImg')||host.querySelector('img.v255ControlImg')||host.querySelector('img.v252ControlImg');if(!img){host.textContent='';img=document.createElement('img');host.appendChild(img)}img.className='v257ControlImg';if(img.getAttribute('src')!==src)img.src=src;img.alt=alt;img.draggable=false;return true}
...
function retireKnownBlocker(){const look=$('#lookPad');if(look){look.style.display='none';look.style.pointerEvents='none';look.dataset.v257Retired='1'}}
function syncVersionLabel(){const meta=document.querySelector('.brandMetaV250 span:first-child');if(meta)meta.textContent='V2.5.7 · 5D K線西遊記'}
