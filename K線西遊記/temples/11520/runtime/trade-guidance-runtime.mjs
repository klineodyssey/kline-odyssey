...
export function buildOrderSpeech({stage,axis,symbol,side,lots,c,margin,freeKgen,pnl,maxAdversePoints,reason}){
  ...
  if(stage===TRADE_GUIDE_STATE.PREVIEW)return `${axis} ${symbol}，${side}，${lots}口，槓桿 ${speed} C（上限 100C）。本金與保證金 ${m} KGEN。標的每變動 1%，未實現損益約為 ${lots} × ${speed} × 1% KGEN${risk}。確認後才會進入送單程序。`;
...
export const CUSTOMER_SERVICE_TOPICS = Object.freeze({
  ...
  LEVERAGE:'11520 的槓桿是 C，絕對值上限 100C。C 不降低本金；下 100 口需要 100 KGEN 本金。未實現損益 = 價格報酬率 × 口數 × C；100C 時反向 1% 會使隔離本金歸零。',
  ...
  RISK:'C 是槓桿倍率，不是本金折扣，絕對值上限 100C。單筆最大損失限制在該筆本金；本金耗盡即清算，不再向錢包追繳本單之外的 KGEN。'
});
