...
function style(){
  ...
  /* Joystick hit-box stays unchanged; only visible rings shrink. */
  ...
  /* Wallet collapse keeps its handle at the same screen position. */
  ...
  /* Clean play mode: 3D world first; settings become an explicit layer. */
  ...
  body.game-clean-mode .assetTag{opacity:.3}
  body.game-clean-mode #gameModeToggle{display:block!important}
  body:not(.game-clean-mode) #gameModeToggle{display:block}
...
function installEnergyLayers(){
  const cLevels=[0,.000001,.00001,.0001,.001,.01,.1,1,10,100,1000],syncers=[];
  const ratioFor=control=>{
    ...
    if(control.id==='cControl'){
      const raw=document.getElementById('cRead')?.textContent||'0',v=parseFloat(raw.replace('C',''))||0;
      let i=cLevels.findIndex(x=>Math.abs(x-v)<=Math.max(1e-12,Math.abs(v)*1e-8));if(i<0)i=0;return i/(cLevels.length-1);
    }
    return 0;
  };
  for(const control of document.querySelectorAll('.sliderDock .v')){
    ...
    let last=-1;const sync=()=>{const ratio=ratioFor(control);if(Math.abs(ratio-last)<.0001)return;last=ratio;fill.style.height=`${(ratio*100).toFixed(1)}%`;control.dataset.energy=ratio>.88?'max':ratio>.62?'high':'normal';if(control.id==='yControl')control.dataset.airborne=ratio>0?'1':'0';};
    syncers.push(sync);const read=control.querySelector('.read');if(read)new MutationObserver(sync).observe(read,{childList:true,characterData:true,subtree:true});
    control.addEventListener('pointerdown',sync);control.addEventListener('pointermove',sync);control.addEventListener('pointerup',sync);addEventListener('resize',sync);sync();
  }
  let lastFrame=0;const liveSync=now=>{if(now-lastFrame>50){for(const fn of syncers)fn();lastFrame=now}requestAnimationFrame(liveSync)};requestAnimationFrame(liveSync);
}

function installPlayMode(){
  if(document.getElementById('gameModeToggle'))return;
  const btn=document.createElement('button');btn.id='gameModeToggle';btn.type='button';btn.textContent='⚙';btn.setAttribute('aria-label','切換遊戲畫面與設定畫面');document.body.appendChild(btn);
  const key='11520.play.cleanMode';let clean=localStorage.getItem(key)!=='0';
  const render=()=>{document.body.classList.toggle('game-clean-mode',clean);btn.textContent=clean?'⚙':'🎮';btn.title=clean?'開啟設定/控制面板':'回到正式遊戲畫面';};
  btn.onclick=()=>{clean=!clean;localStorage.setItem(key,clean?'1':'0');render()};render();
}
...
function installDeadZoneCamera(){
  ...
  let focus=null;const clamp=(v,a,b)=>Math.max(a,Math.min(b,v));
  proto.lookAt=function(x,y,z){
    ...
    const ox=this.position.x-tx,oz=this.position.z-tz,len=Math.hypot(ox,oz)||1,back={x:ox/len,z:oz/len},forward={x:-back.x,z:-back.z},right={x:back.z,z:-back.x};
    const dx=tx-focus.x,dz=tz-focus.z,sx=dx*right.x+dz*right.z,sz=dx*forward.x+dz*forward.z,deadX=2.65,deadZ=2.15,cx=clamp(sx,-deadX,deadX),cz=clamp(sz,-deadZ,deadZ);
    const desired={x:tx-(right.x*cx+forward.x*cz),z:tz-(right.z*cx+forward.z*cz)};focus.x+=(desired.x-focus.x)*.34;focus.z+=(desired.z-focus.z)*.34;
    const shiftX=focus.x-tx,shiftZ=focus.z-tz;this.position.x+=shiftX;this.position.z+=shiftZ;return originalLookAt.call(this,focus.x,ty,focus.z);
...
function installGameChat(){
  ...
  const key='11520.chat.v1';let channel='world';let store;try{store=JSON.parse(localStorage.getItem(key)||'{}')}catch{store={}}for(const c of ['world','team','system'])if(!Array.isArray(store[c]))store[c]=[];
...
function bestProvider(){const providers=[];const add=p=>{if(p?.request&&!providers.includes(p))providers.push(p)};add(globalThis.ethereum);for(const p of globalThis.ethereum?.providers||[])add(p);add(globalThis.trustwallet?.ethereum);add(globalThis.trustwallet);add(globalThis.BinanceChain);add(globalThis.okxwallet);add(globalThis.bitkeep?.ethereum);return providers[0]||null;}
function siteRootPath(){const path=decodeURI(location.pathname||''),marker='/K線西遊記/',i=path.indexOf(marker);return i>=0?path.slice(0,i):'';}
function wallet12345BridgeUrl(){const root=siteRootPath(),u=new URL(`${root}/wallet-12345.html`,location.origin);u.searchParams.set('autoconnect','1');u.searchParams.set('source','11520');u.searchParams.set('from','game-5d');u.searchParams.set('bridge','1');return u.href;}
...
function installWalletLauncher(){
  ...
  sheet.querySelector('#open12345Wallet').href=wallet12345BridgeUrl();sheet.querySelector('#close11520WalletSheet').onclick=()=>sheet.classList.remove('open');
  ...
  let retrying=false;const retry=()=>{if(retrying||!bestProvider())return;retrying=true;sheet.classList.remove('open');queueMicrotask(()=>connect.click());setTimeout(()=>{retrying=false},600)};addEventListener('focus',retry);document.addEventListener('visibilitychange',()=>{if(!document.hidden)retry()});
}
...
export function install11520MobileShell(){
  ...
  installEnergyLayers();installPlayMode();installDeadZoneCamera();installGameChat();installWalletLauncher();
}
