...
export function previewOrder({axis,symbol,fire,leverage,price,kgen,hasPosition}){const signedLots=Number(fire),lots=Math.abs(signedLots),c=Math.abs(Number(leverage));if(!Number.isFinite(signedLots)||signedLots===0)return{ok:false,reason:'ZERO_FIRE'};if(!Number.isInteger(lots)||lots>100)return{ok:false,reason:'BAD_LOTS'};if(!Number.isFinite(c)||c<1||c>100)return{ok:false,reason:'BAD_C'};if(!Number.isFinite(Number(price))||Number(price)<=0)return{ok:false,reason:'NO_PRICE'};if(hasPosition)return{ok:false,reason:'POSITION_EXISTS'};const side=signedLots>0?'多':'空',im=requiredMargin({lots}),mm=im*.75;if(!Number.isFinite(Number(kgen))||Number(kgen)<im)return{ok:false,reason:'INSUFFICIENT_KGEN',im};return{ok:true,order:{axis,symbol,side,lots,c,L:c,price:Number(price),im,mm}}}
export function executeOrder(state,order){if(!order)return{ok:false,reason:'NO_ORDER'};if(state.kgen<order.im)return{ok:false,reason:'INSUFFICIENT_KGEN'};state.kgen-=order.im;state.pos[order.axis]={side:order.side,lots:order.lots,c:order.c??order.L,leverage:order.c??order.L,entry:order.price,symbol:order.symbol,margin:order.im,maintenance:order.mm};state.history.unshift({time:new Date().toLocaleTimeString(),axis:order.axis,symbol:order.symbol,event:`${order.side}開倉`,pnl:0});return{ok:true}}
export function positionPnl(position,quote){if(!position)return 0;return pnlForMove({entry:position.entry,mark:quote??position.entry,side:position.side,lots:position.lots,c:position.c??position.leverage})}
