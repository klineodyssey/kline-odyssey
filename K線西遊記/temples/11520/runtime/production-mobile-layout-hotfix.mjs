...
function enforce(){
  ...
  ensureStyle();
  for(const sel of ['.hud-drawer-toggle','#hudToggleAxes','#hudToggleTele','#hudToggleMonster','#hudToggleMap','#hudToggleParams','#yJoyV250'])for(const el of document.querySelectorAll(sel))hideLegacy(el);
  const rails=[['#cControl','166px'],['#lotsControl','216px'],['#yControl','266px']];
  for(const [sel,left] of rails){const el=$(sel);if(!el)continue;for(const [k,v] of [['position','fixed'],['left',left],['right','auto'],['top','auto'],['bottom','max(28px, env(safe-area-inset-bottom))'],['width','42px'],['height','132px'],['transform','none'],['display','block'],['visibility','visible'],['opacity','1'],['pointer-events','auto'],['z-index','756']])if(el.style.getPropertyValue(k)!==v||el.style.getPropertyPriority(k)!=='important')el.style.setProperty(k,v,'important')}
  const chat=$('#chatHandle');if(chat){for(const [k,v] of [['position','fixed'],['left','auto'],['right','5px'],['top','auto'],['bottom','350px'],['width','42px'],['height','42px'],['z-index','9978']])if(chat.style.getPropertyValue(k)!==v||chat.style.getPropertyPriority(k)!=='important')chat.style.setProperty(k,v,'important')}
  document.documentElement.dataset.k11520ProductionMobileLayout='HOTFIX_V1_0_1';
  return true;
}
...
export function install11520ProductionMobileLayoutHotfix(){
  ...
  ensureStyle();enforce();
  let pending=false;
  const schedule=()=>{if(pending)return;pending=true;setTimeout(()=>{pending=false;enforce()},0)};
  const observer=new MutationObserver(mutations=>{
    if(mutations.some(m=>m.type==='childList'&&[...m.addedNodes].some(n=>n?.nodeType===1)))schedule();
  });
  ...
  for(const delay of [80,220,500,1000,1800,3000])setTimeout(enforce,delay);
  return true;
}
