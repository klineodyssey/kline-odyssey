...
function ensureStyle(){
  ...
  #walletPanel.collapsed #walletToggle{display:grid!important;position:static!important;inset:auto!important;place-items:center!important;width:38px!important;height:38px!important;min-width:38px!important;min-height:38px!important;margin:0!important;padding:0!important;opacity:1!important;visibility:visible!important;color:#9eeeff!important;background:#101923!important;border:1px solid #68e4ff66!important;border-radius:10px!important;font-size:18px!important;line-height:1!important;z-index:1!important}
...
function syncVersion(){const n=document.querySelector('.brandMetaV250 span:first-child');if(n)n.textContent='V2.5.8 · 5D K線西遊記'}
function hardenWallet(){const p=$('#walletPanel'),t=$('#walletToggle');if(!p||!t)return false;if(p.classList.contains('collapsed')){t.textContent='▶';t.setAttribute('aria-label','展開 12345 / BSC 錢包');t.title='展開 12345 / BSC 錢包'}else{t.textContent='◀';t.setAttribute('aria-label','收合 12345 / BSC 錢包');t.title='收合 12345 / BSC 錢包'}return true}
function relocateBag(){const all=[...document.querySelectorAll('[data-organ="bag"],button[title*="背包"],button[aria-label*="背包"]')].filter((el,i,a)=>a.indexOf(el)===i);if(!all.length)return false;let bag=all.find(el=>el.classList.contains('bagRelocatedV258'))||all.find(el=>el.classList.contains('bagRelocatedV250'))||all[0];for(const el of all){if(el!==bag){el.classList.remove('bagRelocatedV250','bagRelocatedV258');el.style.display='none'}}bag.classList.add('bagRelocatedV250','bagRelocatedV258');bag.style.display='grid';bag.textContent=(bag.textContent||'').trim()||'🎒';bag.title=bag.title||'背包';bag.setAttribute('aria-label',bag.getAttribute('aria-label')||'背包');if(bag.parentElement!==document.body)document.body.appendChild(bag);return true}
function health(){const p=$('#walletPanel'),t=$('#walletToggle'),b=$('.bagRelocatedV258');const pr=p?.getBoundingClientRect(),tr=t?.getBoundingClientRect(),br=b?.getBoundingClientRect();const report={version:'2.5.8',walletCollapsed:!!p?.classList.contains('collapsed'),walletCompact:!!pr&&pr.width<=50&&pr.height<=50,walletToggleVisible:!!tr&&tr.width>0&&tr.height>0&&(t?.textContent||'').trim().length>0,bagStandalone:!!b&&b.parentElement===document.body,bagVisible:!!br&&br.width>0&&br.height>0};report.ok=Object.values(report).every((v,k)=>k<1||typeof v!=='boolean'||v);globalThis.__K11520_MOBILE_VISUAL_HEALTH__=report;document.documentElement.dataset.v258VisualHealth=report.ok?'PASS':'RED';return report}
function apply(){ensureStyle();syncVersion();hardenWallet();relocateBag();return health()}

export function install11520MobileVisualHardeningV258(){apply();let n=0;const tick=()=>{n++;apply();if(n<36)setTimeout(tick,120)};setTimeout(tick,80);const mo=new MutationObserver(()=>queueMicrotask(apply));mo.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});setTimeout(()=>{try{mo.disconnect()}catch{}apply()},7000);return health()}
