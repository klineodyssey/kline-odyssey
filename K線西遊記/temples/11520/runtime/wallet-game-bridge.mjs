...
const qs=q=>HAS_DOCUMENT?document.querySelector(q):null;

function ensureUi(){
  if(!HAS_DOCUMENT)return false;
  if(qs('#wallet11520')) return true;
  ...
  qs('#walletConnect11520')?.addEventListener('click',connect);
  qs('#walletRefresh11520')?.addEventListener('click',refresh);
  return true;
}

function render(){
  if(!ensureUi())return;
  const addr=qs('#walletAddr11520'),chain=qs('#walletChain11520'),bnb=qs('#walletBnb11520'),kgen=qs('#walletKgen11520'),msg=qs('#walletState11520');
  if(addr)addr.textContent=short(state.account);
  if(chain)chain.textContent=state.chainId??'--';
  if(bnb)bnb.textContent=state.bnb==null?'--':fmt(state.bnb);
  if(kgen)kgen.textContent=state.kgen==null?'--':`${fmt(state.kgen)} KGEN`;
  if(msg){
    msg.textContent=state.status==='CONNECTED'?'已連線｜鏈上餘額只讀｜交易送鏈仍需正式 Settlement 驗證':state.status==='WRONG_CHAIN'?`鏈錯誤：目前 ${state.chainId}，需要 BSC 56`:state.status==='NO_WALLET'?'找不到瀏覽器/手機注入式 EVM 錢包':state.status==='ERROR'?'錢包讀取失敗，請重試':'未連線｜只讀，不會自動送交易';
    msg.style.color=state.status==='CONNECTED'?'#62e99a':state.status==='DISCONNECTED'?'#9faab4':'#ff9b6b';
  }
  if(HAS_DOCUMENT)document.documentElement.dataset.wallet11520=state.status;
  if(HAS_WINDOW)window.dispatchEvent(new CustomEvent('k11520:wallet',{detail:{account:state.account,chainId:state.chainId,bnb:state.bnb,kgen:state.kgen,status:state.status,verified:state.status==='CONNECTED'}}));
}
...
function attachAssetObserver(){
  ...
  const sheet=qs('#organSheet');
  if(!sheet)return;
  const inject=()=>{
    const title=qs('#organTitle')?.textContent||'';
    const body=qs('#organBody');
  ...
  if(typeof MutationObserver!=='undefined')new MutationObserver(inject).observe(sheet,{subtree:true,childList:true,characterData:true,attributes:true});
  if(HAS_WINDOW)window.addEventListener('k11520:wallet',inject);
}

function boot(){if(!HAS_DOCUMENT)return;ensureUi();render();attachAssetObserver()}
if(HAS_DOCUMENT){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot()}
