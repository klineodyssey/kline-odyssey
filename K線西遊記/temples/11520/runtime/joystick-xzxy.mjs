...
PURPOSE: Persistent XZ/XY plane joystick. XZ is ground locomotion on X/Z; XY is flight locomotion on X/Y. XZ preserves the approved KGEN Genesis art; XY uses the approved Chang'e art; C warp uses the approved UFO art. Plane switching is fail-neutral and exposes machine-verifiable motion intent without changing wallet/trade/settlement/chain authority.
*/
...
let motionInput={active:false,x:0,y:0,z:0,screenX:0,screenY:0};
...
function expectedImage(){return mode==='XY'?{src:FAIRY,alt:'XY 飛行模式嫦娥'}:{src:captureApprovedXz(),alt:'XZ 地面模式 KGEN Genesis'}}
...
function exposeMotion(){document.documentElement.dataset.k11520MotionMode=mode==='XY'?'FLIGHT':'GROUND';globalThis.__K11520_JOYSTICK_XZXY__={organ:'XZXY Joystick',version:'2.6.16',mode,persistent:true,horizontal:'X',vertical:mode==='XY'?'Y':'Z',motion:mode==='XY'?'FLIGHT':'GROUND',input:{...motionInput},assets:{XZ:xzApprovedSrc?'approved-embedded-kgen-genesis':KGEN_FALLBACK,XY:FAIRY,warp:UFO},neutralSwitch:true};globalThis.__K11520_JOYSTICK_PLANE__=globalThis.__K11520_JOYSTICK_XZXY__}
function render(){ensureLabels();ensureKnobImage();ensureUfoAsset();const label=$('#k11520PlaneLabel'),top=$('#k11520DirTop'),bottom=$('#k11520DirBottom'),left=$('#k11520DirLeft'),right=$('#k11520DirRight');if(label)label.textContent=mode==='XY'?'XY 飛行 · 左右 X／上下 Y · 點中央圖切換':'XZ 地面 · 左右 X／前後 Z · 點中央圖切換';if(left)left.textContent='X−';if(right)right.textContent='X+';if(top)top.textContent=mode==='XY'?'Y+':'Z+';if(bottom)bottom.textContent=mode==='XY'?'Y−':'Z−';document.documentElement.dataset.k11520JoyPlane=mode;exposeMotion()}
...
function inputFrom(source,active=true){const joy=$('#joy');if(!joy)return motionInput;const r=joy.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dx=source.clientX-cx,dy=source.clientY-cy,dist=Math.hypot(dx,dy),travel=r.width/2*.68,k=Math.min(1,dist/Math.max(1,travel)),sx=dist?dx/dist*k:0,sy=dist?dy/dist*k:0;motionInput={active:!!active&&k>.02,screenX:sx,screenY:sy,x:sx,y:mode==='XY'?-sy:0,z:mode==='XZ'?-sy:0};exposeMotion();return motionInput}
function clearInput(){motionInput={active:false,x:0,y:0,z:0,screenX:0,screenY:0};exposeMotion()}
...
function toggle(source){releaseXY(source);clearInput();resetKnobVisual();mode=mode==='XZ'?'XY':'XZ';saveMode();render();installImageGuard();installAuxGuard();requestAnimationFrame(()=>{resetKnobVisual();render()});flash(mode==='XY'?'XY 飛行模式：左右 X、上下 Y':'XZ 地面模式：左右 X、前後 Z')}
...
function bind(){const joy=$('#joy'),knob=$('#knob');if(!joy||!knob||joy.dataset.k11520PlaneBound)return false;joy.dataset.k11520PlaneBound='1';
  joy.addEventListener('pointerdown',e=>{if(e.__k11520XYProxy)return;inputFrom(e,true);if(inKnob(e.clientX,e.clientY))centerTap={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now()};if(mode!=='XY')return;xyPid=e.pointerId;xyProxyPid=900000+(Number(e.pointerId)||1);queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointerdown',e,xyProxyPid)})},{capture:true,passive:true});
  joy.addEventListener('pointermove',e=>{if(e.__k11520XYProxy)return;inputFrom(e,true);if(centerTap&&centerTap.id===e.pointerId&&Math.hypot(e.clientX-centerTap.x,e.clientY-centerTap.y)>8)centerTap=null;if(mode!=='XY'||e.pointerId!==xyPid)return;queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointermove',e,xyProxyPid)})},{capture:true,passive:true});
  const end=e=>{if(e.__k11520XYProxy)return;const tap=centerTap;centerTap=null;const isTap=!!tap&&tap.id===e.pointerId&&Math.hypot(e.clientX-tap.x,e.clientY-tap.y)<=8&&performance.now()-tap.t<420;if(isTap){e.preventDefault();toggle(e)}else if(e.pointerId===xyPid){const pid=xyProxyPid;xyPid=null;xyProxyPid=null;queueMicrotask(()=>yEvent(e.type==='pointercancel'?'pointercancel':'pointerup',e,pid||999999))}clearInput();requestAnimationFrame(resetKnobVisual)};
