...
function bind(){const joy=$('#joy'),knob=$('#knob');if(!joy||!knob||joy.dataset.k11520PlaneBound)return false;joy.dataset.k11520PlaneBound='1';knob.addEventListener('pointerdown',e=>{lastKnobDown={id:e.pointerId,x:e.clientX,y:e.clientY,t:performance.now()}},{capture:true,passive:true});knob.addEventListener('pointerup',e=>{const s=lastKnobDown;lastKnobDown=null;if(!s||s.id!==e.pointerId)return;if(Math.hypot(e.clientX-s.x,e.clientY-s.y)<=8&&performance.now()-s.t<420){e.preventDefault();e.stopPropagation();toggle()}},{capture:true,passive:false});knob.addEventListener('pointercancel',()=>{lastKnobDown=null},{capture:true});joy.addEventListener('pointerdown',e=>{if(mode!=='XY'||e.__k11520XYProxy)return;xyPid=e.pointerId;xyProxyPid=900000+(Number(e.pointerId)||1);queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointerdown',e,xyProxyPid)})},{capture:true,passive:true});joy.addEventListener('pointermove',e=>{if(mode!=='XY'||e.__k11520XYProxy||e.pointerId!==xyPid)return;queueMicrotask(()=>{if(xyPid!==e.pointerId)return;neutralizeZ(e);yEvent('pointermove',e,xyProxyPid)})},{capture:true,passive:true});const end=e=>{if(e.__k11520XYProxy||e.pointerId!==xyPid)return;const pid=xyProxyPid;xyPid=null;xyProxyPid=null;queueMicrotask(()=>yEvent(e.type==='pointercancel'?'pointercancel':'pointerup',e,pid||999999))};joy.addEventListener('pointerup',end,{capture:true,passive:true});joy.addEventListener('pointercancel',end,{capture:true,passive:true});return true}
