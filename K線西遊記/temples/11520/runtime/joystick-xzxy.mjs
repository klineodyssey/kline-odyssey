...
let mode='XZ',xyPid=null,xyProxyPid=null,centerTap=null,imageGuard=null;
...
function toggle(){mode=mode==='XZ'?'XY':'XZ';saveMode();render();installImageGuard();flash(mode==='XY'?'XY 模式：左右 X、上下 Y':'XZ 模式：左右 X、上下 Z')}
...
function install(){style();loadMode();ensureLabels();render();bind();installImageGuard();let n=0;const tick=()=>{n++;render();bind();installImageGuard();if(n<20)setTimeout(tick,180)};setTimeout(tick,80);return globalThis.__K11520_JOYSTICK_XZXY__}
