...
PURPOSE: One lower-right settings console owns HUD visibility. Legacy scattered drawer toggles are retired from the visible UI; chat replaces obsolete Y visibility; wallet and backpack are protected organs and can never be hidden by this console. Formal filename is version-free.
*/
...
#backpackButton.bagRelocatedV258{position:fixed!important;z-index:7050!important;right:5px!important;bottom:84px!important;width:46px!important;height:46px!important;min-width:46px!important;min-height:46px!important;margin:0!important;display:grid!important;place-items:center!important}
@media(max-width:420px){.sliderDock{bottom:222px!important}#k11520UiSettings{top:270px;right:54px;width:220px}}
`;
...
function boot(){style();load();retireLegacyCleanMode();retireLegacyDrawerToggles();installPanel();ensureWallet();ensureBackpack();apply();document.addEventListener('fullscreenchange',syncFullscreen);for(const delay of [120,400,1000,2200])setTimeout(()=>{installPanel();retireLegacyDrawerToggles();ensureWallet();ensureBackpack();apply()},delay);try{globalThis.__K11520_UI_SETTINGS_OBSERVER__?.disconnect()}catch{}let timer=0;const mo=new MutationObserver(mutations=>{const relevant=mutations.some(m=>(m.type==='attributes'&&m.target===document.body&&document.body.classList.contains('game-clean-mode'))||(m.type==='childList'&&[...m.addedNodes,...m.removedNodes].some(criticalNode)));if(!relevant||timer)return;timer=setTimeout(()=>{timer=0;retireLegacyCleanMode();retireLegacyDrawerToggles();installPanel();ensureWallet();ensureBackpack();apply()},100)});mo.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});globalThis.__K11520_UI_SETTINGS_OBSERVER__=mo;return globalThis.__K11520_UI_SETTINGS__}
