...
PURPOSE: Keep KX/KY/KZ market axes separate from autonomous player XYZ space, reserve a clean three-rail mobile control zone, keep the wallet toggle stable in the upper utility rail, and prevent lower-right controls from overlapping the three vertical rails.
*/
...
#walletToggle{position:fixed!important;right:8px!important;top:auto!important;bottom:270px!important;left:auto!important;transform:none!important;z-index:9900!important;width:48px!important;height:48px!important;min-width:48px!important;min-height:48px!important;margin:0!important;background-position:center!important;background-repeat:no-repeat!important;background-size:34px 34px!important}
...
function layoutReport(){const rails=['#cControl','#lotsControl','#yControl'].map(s=>rect($(s))),dock=rect($('#dock')),wallet=walletAnchor();const lowerButtons=['#dock','#walletToggle','#aiChatButton','#bgmButton','#backpackButton'].map(s=>({selector:s,rect:rect($(s))}));const overlaps=lowerButtons.flatMap(b=>rails.map((r,i)=>({button:b.selector,rail:i,overlap:overlapRect(b.rect,r)}))).filter(x=>x.overlap);if(!walletInitial&&wallet)walletInitial={...wallet};const walletStable=!!walletInitial&&!!wallet&&Math.abs(walletInitial.x-wallet.x)<1&&Math.abs(walletInitial.y-wallet.y)<1;const railWidths=rails.filter(Boolean).map(r=>r.width);const standardRails=railWidths.length===3&&railWidths.every(w=>w>=46);const out={version:'1.5.0',rails,dock,overlaps,walletAnchor:wallet,walletInitial,walletStable,walletViewportPinned:$('#walletToggle')?.parentElement===document.body,standardRails,cleanThreeRailZone:overlaps.length===0};const status=out.cleanThreeRailZone&&walletStable&&out.walletViewportPinned&&standardRails?'PASS':'RED';document.documentElement.dataset.k11520MarketOriginLayout=status;document.documentElement.dataset.k11520WalletLayout=status;document.documentElement.dataset.k11520ThreeRailLayout=status;globalThis.__K11520_MARKET_ORIGIN_LAYOUT__=out;globalThis.__K11520_WALLET_ANCHOR__={initial:walletInitial,current:wallet,stable:walletStable,viewportPinned:out.walletViewportPinned};return out}
