...
const FLAG='__k11520LifeVisualBootstrapV4';
const RENDER_FLAG='__k11520LifeVisualBootstrapRenderV4';
const SCENE_FLAG='__k11520LifeVisualSceneBoundV4';
const slots=[];
...
function isLegacyRock(obj){return obj?.isMesh&&obj.geometry?.type==='DodecahedronGeometry'&&obj.material?.color?.getHex?.()===0x7b2025}
function bindSlot(index,parent,lifeId,species){
  ...
  Object.assign(parent.userData,detailFromLife(baselineByLifeId.get(slot.lifeId)||{lifeId:slot.lifeId,species:slot.species}),{slotIndex:index,lifeVisual:true});return slot;
}
...
function applyEvent(event){
  ...
  slot.parent.visible=Number(event.vitality??100)>0;slot.species=event.species||slot.species||'MARKET_LIFE';Object.assign(slot.parent.userData,detailFromLife({...event,lifeId:slot.lifeId,species:slot.species}),{lifeVisual:true});replaceLifeVisual(slot.parent,{species:slot.species,lifeId:slot.lifeId});
}
...
function bindExistingScene(scene){
  if(!scene||scene.userData?.[SCENE_FLAG])return;
  const found=[];
  scene.traverse?.(obj=>{if(isLegacyRock(obj)&&!slots.some(s=>s?.parent===obj))found.push(obj)});
  for(const obj of found){const index=slots.length;const initial=INITIAL[index]||[null,'MARKET_LIFE'];bindSlot(index,obj,initial[0],initial[1]);if(index>=INITIAL.length)obj.visible=false}
  flushPending();
  scene.userData=scene.userData||{};scene.userData[SCENE_FLAG]=true;
}
...
export function installLifeVisualBootstrap(){
  ...
  THREE.Scene.prototype.add=function(...objects){
    for(const obj of objects){
      try{if(isLegacyRock(obj)){const index=slots.length;const initial=INITIAL[index]||[null,'MARKET_LIFE'];bindSlot(index,obj,initial[0],initial[1]);if(index>=INITIAL.length)obj.visible=false;flushPending()}}catch{}
    }
    return originalAdd.apply(this,objects);
  };
  if(!THREE.WebGLRenderer.prototype[RENDER_FLAG]){
    const originalRender=THREE.WebGLRenderer.prototype.render;
    THREE.WebGLRenderer.prototype.render=function(scene,camera){try{bindExistingScene(scene)}catch{}return originalRender.call(this,scene,camera)};
    THREE.WebGLRenderer.prototype[RENDER_FLAG]=true;
  }
  ...
  globalThis.K11520LifeVisuals={version:'V4',slots,byLifeId,applyEvent,bindExistingScene,get(lifeId){return byLifeId.get(lifeId)||null}};
  return {ok:true};
}
