...
export const LIFE_VISUAL_VERSION='11520-LIFE-VISUAL-V1';
...
export function decorateLegacyMarketLifeMesh(parent,{species='MARKET_LIFE'}={}){
  if(!parent||parent.userData?.lifeVisualDecorated)return parent;
  parent.userData.lifeVisualDecorated=true;
  if(parent.isMesh){parent.material=new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthWrite:false});parent.castShadow=false;parent.receiveShadow=false;}
  const visual=createLifeVisual(species);visual.position.y=-.10;parent.add(visual);return parent;
}
