...
@media(max-width:${MOBILE_MAX}px){
  ...
  #cControl{position:fixed!important;left:166px!important;right:auto!important;top:auto!important;bottom:max(24px,env(safe-area-inset-bottom))!important;width:44px!important;height:132px!important;z-index:456!important}
  #lotsControl{position:fixed!important;left:214px!important;right:auto!important;top:auto!important;bottom:max(24px,env(safe-area-inset-bottom))!important;width:44px!important;height:132px!important;z-index:456!important}
  #yControl{position:fixed!important;left:262px!important;right:auto!important;top:auto!important;bottom:max(24px,env(safe-area-inset-bottom))!important;width:44px!important;height:132px!important;z-index:456!important;display:block!important}
...
function applyRail(){const el=$('#yControl');if(!el||innerWidth>MOBILE_MAX)return;for(const [k,v] of [['position','fixed'],['left','262px'],['right','auto'],['top','auto'],['bottom','max(24px, env(safe-area-inset-bottom))'],['width','44px'],['height','132px'],['display','block'],['transform','none'],['margin','0'],['z-index','456'],['opacity','1'],['visibility','visible'],['pointer-events','auto']])setImportant(el,k,v);el.dataset.k11520MobileLayout='remaining-axis-rail'}
...
function groupedRails(warp,lots,axis){if(!warp||!lots||!axis)return false;const gap1=lots.left-warp.right,gap2=axis.left-lots.right;return Math.abs(gap1-4)<=1&&Math.abs(gap2-4)<=1&&Math.abs(warp.bottom-lots.bottom)<=2&&Math.abs(lots.bottom-axis.bottom)<=2}
function insideViewport(...items){return items.filter(Boolean).every(r=>r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight)}
function measure(){const report={viewport:{width:innerWidth,height:innerHeight},joy:rect('#joy'),axisRail:rect('#yControl'),warp:rect('#cControl'),lots:rect('#lotsControl'),attack:rect('#attack'),order:rect('#orderFire'),minimap:rect('.minimapWrap'),world:rect('.tele'),life:rect('.monsterHud'),wallet:rect('#walletPanel'),backpack:rect('#backpackButton'),dock:rect('#dock'),master:rect('#hudMasterToggle'),signedEnergy:signedEnergy(),masterCollapseQa:globalThis.__K11520_MASTER_COLLAPSE_QA__||null};report.overlaps={warpMinimap:overlap(report.warp,report.minimap),lotsMinimap:overlap(report.lots,report.minimap),warpLots:overlap(report.warp,report.lots),railDock:overlap(report.axisRail,report.dock),attackWarp:overlap(report.attack,report.warp),orderLots:overlap(report.order,report.lots),worldLife:overlap(report.world,report.life)};report.equalWorldLifeWidth=!report.world||!report.life||Math.abs(report.world.width-report.life.width)<=2;report.railGrouped=groupedRails(report.warp,report.lots,report.axisRail);report.verticalControlsInsideViewport=insideViewport(report.warp,report.lots,report.axisRail);report.ok=innerWidth>MOBILE_MAX||(Object.values(report.overlaps).every(v=>!v)&&report.equalWorldLifeWidth&&report.railGrouped&&report.verticalControlsInsideViewport&&(report.masterCollapseQa?.ok!==false));document.documentElement.dataset.k11520MobileControlLayout=report.ok?'PASS':'RED';return report}
