...
} from "../../../core/index.mjs?v=11520-v3.5-gatekeeper-duty";
import { readTempleHeart12345 } from "../../../core/integrations/temple-heart-12345.mjs?v=11520-v3.5-gatekeeper-duty";
...
function workerStatusMarkup() {
  ...
  return `<div class="grid two">
    ...
    <article class="card"><div class="eyebrow">PRIMARY JOB · 12345 GATEKEEPER</div><h3>${badge(duty.status ?? "NOT_STARTED")}</h3>${kv("Last patrol", duty.gatekeeper_finished_at ?? "NO_SHARED_EVIDENCE")}${kv("Heart block", duty.heart_block)}${kv("Core Heart Indexer", duty.claim_monitor_status ?? heart.core_heart_indexer ?? "UNAVAILABLE")}${kv("Advanced Graph", duty.advanced_graph_status ?? heart.advanced_graph_indexer ?? "INDEXER_REQUIRED_OPTIONAL")}${action("Heartbeat")}${action("Fortune")}${action("Ignition")}${action("Lamp")}${action("Wish")}${action("Thanksgiving")}${kv("Eligibility", heart.eligibility_source ?? "CLIENT_DERIVED")}${kv("Heart write", "WRITE_NOT_CONNECTED")}</article>
...
async function homeView() {
  ...
  return `${hero("K11520 · UNIVERSAL MARKET RUNTIME V3.5", uiLocale === "zh-TW" ? "文明資產的公開市場與生命工廠。" : "A public market and Life Factory for civilization assets.", uiLocale === "zh-TW" ? "數位螞蟻先完成悟空守門本職，再進行公司工作；所有狀態以共享證據為準。" : "The Digital Ant completes Wukong Gatekeeper duty before company work; all status is evidence-backed.")}
...
async function lifeDetailView(lifeId) {
  ...
  const timelineRows = life.life_id === "DIGITAL_ANT_0001" ? [
    ...
    ["First Heartbeat", null, liveEventStatus.FIRST_HEARTBEAT_EVENT ?? "NOT YET"], ["First Fortune", null, liveEventStatus.FIRST_FORTUNE_EVENT ?? "NOT YET"],
    ["First KGEN", null, liveEventStatus.FIRST_KGEN_EVENT ?? "NOT YET"], ["First Wish", null, liveEventStatus.FIRST_WISH_EVENT ?? "NOT YET"],
  ...
  return `${hero("AI LIFE PROFILE", life.life_id, "Canonical identity persists across App versions. Public birth evidence is visible; private signer material never enters the static frontend.")}
    ...
    ${life.life_id === "DIGITAL_ANT_0001" ? section("PRIMARY JOB · WUKONG GATEKEEPER", `<div class="grid two"><article class="card"><div class="eyebrow">WUKONG_GATEKEEPER</div><h3>${badge(liveDuty?.status ?? "NOT_STARTED")}</h3>${kv("Duty", sharedWorkerStatus?.worker_health === "HEALTHY" ? "ON DUTY" : sharedWorkerStatus?.worker_health ?? "EVIDENCE_PENDING")}${kv("Last Gatekeeper Patrol", liveDuty?.gatekeeper_finished_at ?? "NO_V3_5_SHARED_EVIDENCE_YET")}${kv("Gatekeeper Health", liveDuty?.status ?? "NOT_STARTED")}${kv("Heart Block", liveDuty?.heart_block)}${kv("Core Heart Indexer", liveDuty?.claim_monitor_status ?? "EVIDENCE_PENDING")}${kv("Advanced Graph", liveDuty?.advanced_graph_status ?? "INDEXER_REQUIRED_OPTIONAL")}${kv("Secure Signer", "NOT_CONNECTED")}</article><article class="card"><div class="eyebrow">HEART ACTIONS · READ ONLY</div>${kv("Heartbeat", sharedWorkerStatus?.patrols?.temple_12345?.heartbeat?.status ?? "UNAVAILABLE")}${kv("Fortune", sharedWorkerStatus?.patrols?.temple_12345?.fortune?.status ?? "UNAVAILABLE")}${kv("Ignition", sharedWorkerStatus?.patrols?.temple_12345?.ignition?.status ?? "UNAVAILABLE")}${kv("Lamp", sharedWorkerStatus?.patrols?.temple_12345?.lamp?.status ?? "UNAVAILABLE")}${kv("Wish", sharedWorkerStatus?.patrols?.temple_12345?.wish?.status ?? "UNAVAILABLE")}${kv("Vow / Thanksgiving", sharedWorkerStatus?.patrols?.temple_12345?.thanksgiving?.status ?? "NOT_ELIGIBLE")}${kv("Write", "WRITE_NOT_CONNECTED")}</article><article class="card"><div class="eyebrow">SECONDARY WORK</div><h3>AI ANT COMPANY FOUNDER</h3>${kv("Company Patrol", sharedWorkerStatus?.patrols?.company?.status ?? "PENDING_PRIMARY_JOB")}${kv("Primary Job Gate", liveDuty?.status === "COMPLETED" || (liveDuty?.status === "DEGRADED" && liveDuty?.degradation_affects_safety === false) ? "PASSED" : "NOT_PASSED")}${kv("Real Customers", sharedWorkerStatus?.request_patrol?.real_requests ?? 0)}</article></div>`) : ""}
