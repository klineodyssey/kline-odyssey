...
await fs.writeFile(`${OUT}/11520-xz-xy-layout.json`,JSON.stringify({capturedAt:new Date().toISOString(),mode:await page.locator('html').getAttribute('data-k11520-joy-plane'),xyz:await xyz(),version:await page.locator('.brandMetaV250 span:first-child').textContent().catch(()=>null)},null,2));
await browser.close();
console.log('11520 XZ/XY browser QA PASS');
