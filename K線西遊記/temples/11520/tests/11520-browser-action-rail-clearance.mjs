...
await page.waitForFunction(()=>globalThis.__K11520_ACTION_RAIL_CLEARANCE__?.version==='1.2.2',null,{timeout:3000});
...
assert.equal(r.actionsSeparated,true,'attack/order controls must stay separated');
...
const walletBefore=await page.locator('#walletToggle').boundingBox();assert.ok(walletBefore,'wallet toggle missing');
await page.locator('#walletToggle').click({force:true});await page.waitForTimeout(180);const walletAfterOne=await page.locator('#walletToggle').boundingBox();
await page.locator('#walletToggle').click({force:true});await page.waitForTimeout(180);const walletAfterTwo=await page.locator('#walletToggle').boundingBox();
...
await page.evaluate(()=>document.querySelector('#confirm')?.classList.add('open'));await page.waitForTimeout(180);
...
assert.ok(layer.z>layer.hudMax,`open order/confirm surface must be top layer: ${JSON.stringify(layer)}`);assert.equal(layer.hit,true,'open order/confirm surface must receive pointer hit testing above HUD');
await browser.close();console.log('11520 action/rail clearance PASS: live dock geometry, fixed wallet anchor, separated actions, topmost action surface');
