...
const identity=await page.evaluate(async()=>{
  ...
  const title=document.createElement('h2');title.textContent='11520 同一物品・世界狀態 + Digital Ant Live Cargo';title.style.cssText='font-size:15px;color:#f5d77c;text-align:center;margin:2px 0 8px';host.appendChild(title);
  ...
  for(const context of WORLD_ITEM_CONTEXTS){
    ...
    const camera=new THREE.PerspectiveCamera(34,150/130,.01,20);camera.position.set(1.7,1.25,2.15);camera.lookAt(0,0,0);
    ...
    const small=document.createElement('small');small.textContent=`${visual.descriptor.shape} · ${visual.descriptor.label}`;small.style.cssText='display:block;color:#7fa5b5;margin-top:1px;font-size:8px';card.appendChild(small);grid.appendChild(card);
  }
  ...
  syncLifeVisual(ant,transit);const cargo=ant.getObjectByName('LIFE_STATUS_CARGO');const transitIdentity=cargo.userData.cargoIdentityKey,transitShape=cargo.userData.itemShape,transitContext=cargo.userData.cargoContext;
  ...
  const antCard=document.createElement('article');antCard.style.cssText='grid-column:1/-1;border:1px solid #68e4ff55;border-radius:10px;background:#0a1720;padding:5px;text-align:center';antCanvas.style.cssText='width:100%;height:180px;object-fit:contain';antCard.appendChild(antCanvas);const antLabel=document.createElement('b');antLabel.textContent=`LIVE ANT · ${transitContext} · ${transitShape}`;antLabel.style.cssText='display:block;color:#68e4ff;font-size:10px';antCard.appendChild(antLabel);grid.appendChild(antCard);
  const waiting={...transit,mission:{status:'ARRIVED_AWAITING_RECEIPT'}};syncLifeVisual(ant,waiting);const unloadIdentity=cargo.userData.cargoIdentityKey,unloadShape=cargo.userData.itemShape,unloadContext=cargo.userData.cargoContext;
  document.body.appendChild(host);
  return{keys,contexts:[...WORLD_ITEM_CONTEXTS],transitIdentity,unloadIdentity,transitShape,unloadShape,transitContext,unloadContext};
});
...
console.log(`[11520 WORLD ITEM IDENTITY QA] PASS contexts=${identity.contexts.join(',')} liveCargo=${identity.transitContext}->${identity.unloadContext} screenshot=${worldOut}`);
await browser.close();
