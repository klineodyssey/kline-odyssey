...
async function snapshot(page){return page.evaluate(sels=>{
  ...
  const compactNormal=innerWidth<=600&&!document.documentElement.classList.contains('k11520ImmersiveViewport');
  return{width:innerWidth,height:innerHeight,compactNormal,boxes:Object.fromEntries(sels.map(s=>[s,box(document.querySelector(s))])),cards:[...document.querySelectorAll('#axes .axis')].map(box),balances:[...document.querySelectorAll('.top>.pill')].map(box),drawers:[...document.querySelectorAll('.hud-drawer-toggle')].map(box),settingsInstalled:!!globalThis.__K11520_UI_SETTINGS__,layoutInstalled:!!globalThis.__K11520_MOBILE_CONTROL_LAYOUT__,xyzInstalled:!!globalThis.__K11520_3D_CONTROL__,utilityOpen:document.documentElement.classList.contains('k11520UtilitiesOpen'),version:document.querySelector('.brandMetaV250')?.textContent};
},selectors)}
function check(label,state,{expanded=false}={}){const b=state.boxes;const ok=(value,message)=>{if(!value)failures.push(`${label}: ${message}`)};
  ...
  for(const s of ['.top','#joy','#cControl','#lotsControl','#yControl','#cThumb','#lotsThumb','#yThumb','#k11520UtilityMaster','#orderFire','#attack']){const r=b[s];ok(r?.visible,`${s} missing/hidden`);if(r?.visible)ok(r.x>=-1&&r.y>=-1&&r.right<=state.width+1&&r.bottom<=state.height+1,`${s} outside viewport`)}
  if(state.compactNormal){for(const s of ['.axes','.tele','.monsterHud','.minimapWrap'])ok(!b[s]?.visible,`${s} must stay hidden in compact normal mode`);ok(b['.top']?.height<=36,'compact normal system line must stay <=36px')}
  ...
  const clock=b['#brandClockV250'];if(clock?.visible){ok(clock.right<=Math.min(...state.balances.map(r=>r.x))-4,'header clock crosses into balances');ok(clock.bottom<=b['.top'].bottom-4,'header clock escapes header')}
  ...
  if(!state.compactNormal){const maxCard=Math.max(...state.cards.map(r=>r.bottom));for(const s of ['.tele','.monsterHud'])if(b[s]?.visible){ok(b[s].y>=maxCard+6,`${s} overlaps actual market cards`);ok(b[s].scroll<=b[s].client+1,`${s} clips text`)}}
