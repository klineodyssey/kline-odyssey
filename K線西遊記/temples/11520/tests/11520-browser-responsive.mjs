...
// Exercise the actual entry, without importing/repairing UI modules from the test.
...
const profiles=PRODUCTION?[{name:'pages-390',width:390,height:844},{name:'pages-432',width:432,height:856}]:[
  {name:'cold-390',width:390,height:844},
  {name:'cold-432',width:432,height:856},
  {name:'warm-412',width:412,height:772,warm:true},
  {name:'cold-360',width:360,height:740},
...
await fs.mkdir(`${OUT}/public-browser-dependencies`,{recursive:true});
const browser=await chromium.launch({headless:true});
const reports=[],failures=[],assets=new Map(),downloads=[];
const sha=value=>createHash('sha256').update(value).digest('hex');
function publicDependency(url){return url.startsWith('https://cdn.jsdelivr.net/npm/three@0.180.0/')||url==='https://raw.githubusercontent.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0/main/addons/kaykit_character_pack_adventures/Characters/gltf/Knight.glb';}
async function snapshot(page){return page.evaluate(sels=>{
  const box=el=>{if(!el)return null;const r=el.getBoundingClientRect(),s=getComputedStyle(el);const visible=s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)>0&&r.width>0&&r.height>0;const h=visible?document.elementFromPoint(r.x+r.width/2,r.y+r.height/2):null;return{x:r.x,y:r.y,right:r.right,bottom:r.bottom,width:r.width,height:r.height,visible,hit:!!h&&(h===el||el.contains(h)),scroll:el.scrollHeight,client:el.clientHeight,text:(el.textContent||'').trim().slice(0,240)}};
...
function check(label,state){const b=state.boxes;const requireOk=(ok,message)=>{if(!ok)failures.push(`${label}: ${message}`)};
  for(const s of ['.top','.tele','.monsterHud','.minimapWrap','#joy','#cControl','#lotsControl','#yControl','#cThumb','#lotsThumb','#yThumb','#k11520UtilityMaster','#orderFire','#attack']){const r=b[s];requireOk(r?.visible,`${s} missing/hidden`);if(r?.visible)requireOk(r.x>=-1&&r.y>=-1&&r.right<=state.width+1&&r.bottom<=state.height+1,`${s} outside viewport`)}
  requireOk(state.drawers.every(r=>!r?.visible),'legacy drawer controls visible');
  const maxCard=Math.max(...state.cards.map(r=>r.bottom));
  for(const s of ['.tele','.monsterHud'])if(b[s]){requireOk(b[s].y>=maxCard+6,`${s} overlaps actual market cards`);requireOk(b[s].scroll<=b[s].client+1,`${s} clips text`)}
  const rails=['#cControl','#lotsControl','#yControl'].map(s=>b[s]);if(rails.every(Boolean)){requireOk(rails.every(r=>Math.abs(r.y-rails[0].y)<2),'three rails not aligned');requireOk(rails[0].right+5<=rails[1].x&&rails[1].right+5<=rails[2].x,'three rails overlap')}
  for(const s of ['#cThumb','#lotsThumb','#yThumb'])if(b[s])requireOk(Math.abs(b[s].width-b[s].height)<.5,`${s} is not circular`);
  requireOk(utilities.every(s=>!b[s]?.visible),'collapsed utility lane contains extra controls');
  requireOk(b['#k11520UtilityMaster']?.hit,'utility master blocked');
}
try{for(const profile of profiles){const context=await browser.newContext({viewport:{width:profile.width,height:profile.height},isMobile:true,hasTouch:true});const page=await context.newPage();const errors=[],warnings=[];page.on('pageerror',e=>errors.push(String(e)));page.on('console',m=>{if(['warning','error'].includes(m.type()))warnings.push(m.text().slice(0,300))});
  // Only public Three.js/Knight dependencies are retained for reproducible offline QA.
  page.on('response',response=>{const url=response.url();if(!response.ok()||!publicDependency(url)||assets.has(url))return;assets.set(url,null);downloads.push((async()=>{try{const body=await response.body(),name=sha(url)+(url.endsWith('.glb')?'.glb':'.js');await fs.writeFile(`${OUT}/public-browser-dependencies/${name}`,body);assets.set(url,{name,sha256:sha(body),bytes:body.length})}catch{assets.delete(url)}})())});
  ...
  }catch(error){report.error=String(error);failures.push(`${profile.name}: ${String(error)}`);await page.screenshot({path:`${OUT}/${profile.name}-failure.png`,fullPage:true,timeout:5000}).catch(()=>{})}finally{await Promise.allSettled(downloads);await context.close()}
}}finally{await browser.close();await fs.writeFile(`${OUT}/report.json`,JSON.stringify({capturedAt:new Date().toISOString(),base:BASE,head:process.env.GITHUB_SHA||null,reports,failures},null,2));await fs.writeFile(`${OUT}/public-browser-dependencies/manifest.json`,JSON.stringify(Object.fromEntries([...assets].filter(([,value])=>value)),null,2))}
assert.deepEqual(failures,[],'Responsive product failures; inspect screenshots and report.json');
console.log('11520 real-entry responsive/cold-warm/realistic-quotes/utility cycles PASS');
