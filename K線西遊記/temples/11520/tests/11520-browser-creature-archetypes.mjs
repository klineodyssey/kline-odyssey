...
const result=await page.evaluate(async()=>{
  ...
  const display=s=>s.replace('DIGITAL_','D.').replace('BULL_DEMON','牛魔王').replace('STONE_APE','石猿').replace('FIRE_WISP','火靈');
  ...
  for(let row=0;row<3;row++){
    ...
    const renderer=new THREE.WebGLRenderer({canvas,antialias:true,alpha:false});renderer.setSize(360,190,false);renderer.setPixelRatio(1);renderer.setClearColor(0x0a1720,1);
    const scene=new THREE.Scene();scene.add(new THREE.HemisphereLight(0xffffff,0x263544,2.15));const key=new THREE.DirectionalLight(0xffffff,2.1);key.position.set(3,6,8);scene.add(key);
    const camera=new THREE.OrthographicCamera(-5.2,5.2,2.75,-.2,.1,30);camera.position.set(0,1.35,10);camera.lookAt(0,1.35,0);
    for(let col=0;col<4;col++){
      const i=row*4+col,s=species[i];const root=createProceduralLifeBody(THREE,{species:s,name:s,scale:.72});
    ...
    const legend=document.createElement('div');legend.style.cssText='display:grid;grid-template-columns:repeat(4,1fr);gap:2px;text-align:center;font-size:8px;color:#c7d7df;padding:2px 0 3px';legend.innerHTML=species.slice(row*4,row*4+4).map(s=>`<div>${display(s)}</div>`).join('');wrap.appendChild(legend);host.appendChild(wrap);
  }
