...
const result=await page.evaluate(async()=>{
  ...
  for(const [i,s] of species.entries()){
    ...
    const canvas=document.createElement('canvas');canvas.width=180;canvas.height=180;canvas.setAttribute('aria-label',s);canvas.style.cssText='width:100%;height:168px;display:block;margin:auto';card.appendChild(canvas);
    ...
    const renderer=new THREE.WebGLRenderer({canvas,antialias:true,alpha:false,preserveDrawingBuffer:true});renderer.setSize(180,180,false);renderer.setPixelRatio(1);renderer.setClearColor(0x0a1720,1);renderers.push(renderer);
    ...
    renderer.compile(scene,camera);renderer.render(scene,camera);renderer.getContext().finish();renderer.render(scene,camera);
    labels.push({species:s,archetype:creatureArchetypeForSpecies(s),childCount:root.children.length});
  }
  globalThis.__K11520_QA_CREATURE_RENDERERS__=renderers;
