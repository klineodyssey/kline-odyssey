...
const result=await page.evaluate(async()=>{
  ...
  const frame=(s,i=0)=>{
    ...
    return{root,url:renderCanvas.toDataURL('image/png')};
  };
  frame('DIGITAL_ANT',0); // discard one canonical MeshStandardMaterial capture; headless Chromium's first standard-material readback can be blank.
  const cards=[],labels=[];
  for(const [i,s] of species.entries()){
    const rendered=frame(s,i);
    cards.push({species:s,label:display(s),url:rendered.url});labels.push({species:s,archetype:creatureArchetypeForSpecies(s),childCount:rendered.root.children.length});
  }
