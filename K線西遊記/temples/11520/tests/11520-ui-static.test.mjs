...
test('product display is explicitly versioned as V2.3',()=>{assert.ok(html.includes('<title>11520 花果山 V2.3 · 5D K線西遊記</title>'));assert.ok(html.includes('11520 花果山 V2.3'));assert.ok(html.includes('5D K線西遊記'));assert.ok(fixes.includes('VERSION: 2.3.0'));assert.ok(bootstrap.includes('VERSION: 2.3.1'))});
...
test('living bootstrap has animation, local theme audio, heartbeat, visual AI and speech',()=>{for(const token of ['WORLD HEARTBEAT','lifePulse','themePhrase','pulseHeart','SpeechSynthesisUtterance','goddess-ui.webp','aiAvatar','aiMic','SpeechRecognition','進入活世界','dataset.k11520Life=\'ALIVE\''])assert.ok(bootstrap.includes(token),token)});
test('entry gesture starts theme audio and spoken welcome rather than a silent page',()=>{assert.ok(bootstrap.includes("startLivingAudio();speak('歡迎回到一一五二零花果山"));assert.ok(bootstrap.includes("o.querySelector('.introSkip').onclick"));assert.ok(bootstrap.includes("heartbeatTimer=setInterval(pulseHeart"))});
