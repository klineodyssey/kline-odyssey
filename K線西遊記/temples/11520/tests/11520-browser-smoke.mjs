...
if(await page.locator('#intro11520').count()){
  const enter=page.locator('#intro11520 .introSkip');if(await enter.isVisible())await enter.click({timeout:3000});await page.waitForTimeout(600);
}
...
// Physical upward joystick motion must increase the real world Z coordinate.
...
// Left joystick center controls Y: one tap + one second idle begins Y+; another tap stops it.
...
// Fixed branded icons are independent clickable buttons at the top, not moving thumbs.
...
// World map: empty map tap creates a waypoint first; the user explicitly presses 前往 to start autopilot.
...
// Known map entities expose object info before navigation.
...
// AI and wallet safety surfaces remain reachable.
await assertVisible('aiChatButton');await page.locator('#aiChatButton').click({timeout:3000});await page.waitForTimeout(30);assert.equal(await page.locator('#aiChatPanel').evaluate(el=>el.classList.contains('open')),true);await page.locator('#aiClose').click({timeout:3000});
