...
const motionState=async()=>page.evaluate(()=>({plane:document.documentElement.dataset.k11520JoyPlane,motion:document.documentElement.dataset.k11520MotionMode,avatar:document.documentElement.dataset.k11520AvatarMotion,organ:globalThis.__K11520_JOYSTICK_XZXY__||null}));
...
assert.equal(await page.locator('html').getAttribute('data-k11520-motion-mode'),'GROUND','XZ must expose ground motion mode');
await page.waitForFunction(()=>document.documentElement.dataset.k11520AvatarMotion==='GROUND',{timeout:2500});
await assertKgenArt();
assert.match(await page.locator('#k11520PlaneLabel').textContent(),/XZ 地面/,'XZ label must communicate ground locomotion');
...
assert.equal(await page.locator('html').getAttribute('data-k11520-motion-mode'),'FLIGHT','XY must expose flight motion mode');
await page.waitForFunction(()=>document.documentElement.dataset.k11520AvatarMotion==='FLIGHT',{timeout:2500});
assert.match(await page.locator('#k11520PlaneLabel').textContent(),/XY 飛行/,'XY label must communicate flight locomotion');
...
assert.equal(await page.locator('html').getAttribute('data-k11520-motion-mode'),'GROUND','returning to XZ must restore ground motion mode');
await page.waitForFunction(()=>document.documentElement.dataset.k11520AvatarMotion==='GROUND',{timeout:2500});
...
await page.locator('#k11520UiSettingsClose').click();
...
await fs.writeFile(`${OUT}/11520-xzxy-layout.json`,JSON.stringify({capturedAt:new Date().toISOString(),organ:'XZXY Joystick',mode:await page.locator('html').getAttribute('data-k11520-joy-plane'),xyz:await xyz(),version:await page.locator('.brandMetaV250 span:first-child').textContent().catch(()=>null),motion:await motionState(),uiSettings:await page.evaluate(()=>globalThis.__K11520_UI_SETTINGS__||null),joystick:await page.evaluate(()=>globalThis.__K11520_JOYSTICK_XZXY__||null)},null,2));
await browser.close();
console.log('11520 XZ ground + XY flight + unified UI settings browser QA PASS');
