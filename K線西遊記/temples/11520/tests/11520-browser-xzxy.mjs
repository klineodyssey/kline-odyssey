...
const assertThumbFollows=async()=>{const b=await joyBox(),k0=await knobBox(),c0=center(k0),p0=point(b,.5,.5,1,91),p1=point(b,.88,.22,1,91);await page.dispatchEvent('#joy','pointerdown',p0);await page.dispatchEvent('#joy','pointermove',p1);await page.waitForTimeout(100);const k1=await knobBox(),c1=center(k1),travel=Math.hypot(c1.x-c0.x,c1.y-c0.y),limit=Math.min(b.width,b.height)/2*.70;assert.ok(c1.x>c0.x+12,`joystick thumb must follow finger right: ${JSON.stringify({c0,c1})}`);assert.ok(c1.y<c0.y-10,`joystick thumb must follow finger up: ${JSON.stringify({c0,c1})}`);assert.ok(travel<=limit+2,`joystick thumb must remain clamped: ${JSON.stringify({travel,limit,c0,c1})}`);assert.equal(await page.locator('#knob').getAttribute('data-k11520-disc-drag'),'1','drag visual state missing');await page.screenshot({path:`${OUT}/11520-mobile-joystick-follow.png`,fullPage:true});await page.dispatchEvent('#joy','pointerup',{...p1,buttons:0});await page.waitForTimeout(180);const c2=center(await knobBox());assert.ok(Math.hypot(c2.x-c0.x,c2.y-c0.y)<3,`joystick thumb must ease back to center: ${JSON.stringify({c0,c2})}`);assert.equal(await page.locator('html').getAttribute('data-k11520-disc-thumb-following'),'0','thumb follow state must clear after release')};
