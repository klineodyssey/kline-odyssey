...
- KX/KY/KZ 不只是交易欄位，也是 Market Life 在 K-space 的可活動市場維度。
...
### 5.3 K-space 空間攻擊向量
KX/KY/KZ 同時代表 Market Life 在 K-space 的市場活動能力。多空方向必須可被一眼看見的空間運動表現。

- `KX-`：X 軸由右往左攻擊／移動；`KX+`：X 軸由左往右。
- `KY-`：Y 軸由上往下攻擊／移動；`KY+`：Y 軸由下往上。
- `KZ-`：Z 軸由遠往近攻擊／移動；`KZ+`：Z 軸由近往遠。
- `KX0/KY0/KZ0`：該軸沒有有效市場部位，不畫攻擊向量。

這些向量是 11520 的 Market Life 可視化語義，不取代真實 position/PnL settlement，也不等同現實物理定律。
...
## 6. 一圖一生命｜一眼看懂狀態
每個 Market Life 在 3D 畫面上至少要有一個可縮放的 Life HUD / badge，直接表現：
- 名稱與 LIFE_ID；
- KX/KY/KZ capability 與目前 `+/-/0`；
- 每個有效軸的方向箭頭；
- Life HP/vitality；
- KGEN capital/risk pool；
- strategy / confidence / survival state；
- ALIVE / WOUNDED / RETREATING / DEAD / NAIHE / MENGPO_RECOVERY / REBIRTH；
- 與玩家逐軸 ALIGNED / OPPOSED / NEUTRAL。

出現/消失規則必須可視：
- ALIVE/WOUNDED/RETREATING：世界中可見；
- DEAD：死亡動畫後退出正常戰鬥可見層；
- NAIHE/MENGPO_RECOVERY：不得冒充仍在戰場，可用幽影/奈何標記或從主戰場隱藏；
- REBIRTH：有重生視覺事件後重新進場。

HP=0 與 capital=0 必須分開顯示。`HP=0` 表示生命死亡條件；`capital=0` 表示市場資本耗盡/破產風險條件，AI 可先撤退或被結算規則判定死亡，但 UI 不得把兩者畫成同一條血條。

產品目標：不用開除錯器、不看 JSON，玩家只看一張畫面就知道「這是誰、在哪些 K 軸活動、做多或做空、從哪個方向攻擊、還有多少血、還有多少錢、目前在打/逃/死/奈何/重生」。
...
3. K-SPACE VECTOR：把 `+/-/0` 映射成可視方向，但不自行決定輸贏；
4. CAPITAL/RISK RESULT：KGEN 本金、PnL、風險池變化；
5. LIFE IMPACT：市場結果與世界事件如何影響生命 HP/vitality；
6. KAIOS RESULT：只有符合正式世界事件/擊殺/任務規則才產生 KAIOS；不得用 UI 動畫假造 reward。
...
- market-life-kspace-visual runtime（`+/-/0` -> XYZ/K-space visual vector）
...
- `KX-` 沒有表現為右往左，或其他軸方向與本規格相反；
- 同向部位被誤判成戰鬥；
- 反向部位被誤判成同行；
- 多維怪物只剩單一總體敵友狀態，丟失逐軸關係；
- HP 與 KGEN capital 被畫成同一狀態；
- 玩家看不到怪物出現/死亡/奈何/重生階段；
...
**同方向，是同行；反方向，才是對作戰鬥。KX/KY/KZ 的正負同時就是 Market Life 在 K-space 的可視活動方向。**