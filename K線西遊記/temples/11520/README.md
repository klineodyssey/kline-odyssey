...
- Business approval, funding authority, the technical signer and settlement verification stay separate. Submission requires a repository-owned authority plus an unused `ONE_EXACT_KAIOS_PAYMENT_ACTION` signer policy; both live allowlists are empty. A payment can become paid only after a successful exact-party/token/chain/amount receipt and matching recipient balance delta. The ATM page exposes these gates read-only and cannot sign or transfer.
