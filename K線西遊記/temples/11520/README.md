...
The fixed repository evidence organ is `runtime/gpu-real-evidence-registry.v1.json`. Its closed schema is wired into the GPU readiness evaluator, but the current registry truth is `NO_VERIFIED_REAL_GPU_INVENTORY` with zero records. Callers cannot substitute a path, parser, transport or copied object to manufacture verified inventory. Wiring the verifier removes only the technical `NOT_WIRED` condition; real trade remains blocked by missing independent inventory, ownership, cargo, transport, warehouse, capital, production market, policy, settlement, signer, review and no-broadcast fork evidence.
