# 11520 Universal Exchange V4.0.4
...
- V4.0.4 preserves the mobile injected-wallet entry, fail-closed BSC chain-56 switching and user-initiated wallet display metadata for canonical KGEN and KAIOS, and adds the read-only 11520 settlement code-identity gate. It loads only explicit browser-safe modules; Node-only signer and Starforge modules are excluded from the public browser dependency graph. These controls never receive a private key and connect/add-token/read-only settlement checks send no transaction.
...
- The same page performs a two-endpoint, three-confirmation BSC read of the configured `ExchangeSettlement11520` proxy, implementation slot, fixed 18888 bank, fixed 11520 Brain, module ID, version, governance-finalized flag, governance role and `totalSettled`. Those interface observations do not prove current runtime code identity because expected bytecode hashes are not yet repository-bound. Historical deployment evidence describes V1 as a governance-authorized KAIOS payment from 18888 to the fixed Brain, but the live adapter reports that capability only as an unverified configured claim. In all cases V1 cannot bind a buyer, seller, GPU serial, custody delivery, KGEN/KAIOS dual-quote payment or atomic inventory transfer. The production GPU settlement adapter remains `NOT_IMPLEMENTED` and real trade remains fail-closed.
