...
assert.equal(dispatch.status, "SUSPENDED_BY_HUMAN_COST_DECISION");
...
assert.equal(dispatch.dispatch_mode, "ON_DEMAND_EXTERNAL_CAPACITY_ONLY");
assert.equal(dispatch.external_wake_workflow_allowed, false);
assert.equal(dispatch.cursor_api_key_required, false);
assert.equal(dispatch.automatic, false);
assert.equal(dispatch.external_autonomy, false);
assert.equal(dispatch.bounded_pilot_policy.authorization_evidence, "HUMAN_CURSOR_CLOUD_DEFERRED_UNTIL_HIGH_WORKLOAD_2026-09-13");
