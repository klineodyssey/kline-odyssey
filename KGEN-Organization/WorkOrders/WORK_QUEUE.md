...
| KAIOS-CURSOR-LIFE-ENERGY-PAYROLL-R2-001 | OPEN | UNASSIGNED | codex-gm-01 | P0 | Payroll QA | `NONE` | `KGEN-AI-Company/reports/CURSOR_LIFE_ENERGY_PAYROLL_CANDIDATE_REPORT.md` |
...
- Status: OPEN — expired undelivered claim released; task reassignable
- Owner: UNASSIGNED
...
- Claim release: `CLAIM-EVENT-KAIOS-LIFE-ENERGY-PAYROLL-R2-001-002` (`EXPIRED_UNDELIVERED_RELEASED`, task returned to `OPEN`)
- Superseded draft close: `CLAIM-EVENT-KAIOS-LIFE-ENERGY-PAYROLL-R2-001-003` (preserved history; not current state)
- Employment/task correction: `CLAIM-EVENT-KAIOS-LIFE-ENERGY-PAYROLL-R2-001-004` (`HUMAN_EMPLOYMENT_CORRECTION_REOPENED_REASSIGNABLE`)
- Delivery / Review / Acceptance: `NONE / NONE / NONE`
- Payroll / Payment: `NOT_ELIGIBLE_NO_ACCEPTED_DELIVERY / NOT_SENT`
- Employment: `cursor-01` is `ACTIVE_SUSPENDED_UNPAID / T2`; runtime is `OFFLINE_EXTERNAL_SERVICE_UNAVAILABLE`; payroll is `SUSPENDED_NO_WORK_NO_PAY`.
- Reassignment: permitted to another eligible worker under current task policy. Do not revive the expired Cursor claim. Cursor reactivation requires service availability, Boot, CURRENT sync, registry check, ACK revalidation if required and a fresh claim.
...
- Reconciliation: no R2 delivery, review, acceptance, payroll eligibility or payment. Preserve R1/R2 and superseded employment history; no payment, escrow release, merge, Canon change, contract change or Mainnet write.
